...
const HELPERS = function () {
  ...
  /* The SHORTEST road, not the first one found. Since buildings became solid
   * (2026-08-01) a road can detour a long way around a castle, and the first
   * legal spot on a lattice sweep was coming back with a 16-edge road for a
   * 10-step hop — which silently blew every fixed tick budget below. Picking
   * by road length keeps these tests measuring priority, not walking. */
  T.flagSpot = function (minD, maxD) {
    ...
    FSMap.forRadius(G.map, T.castle().v, maxD || 9, (u, d) => {
      ...
      const p = FS.FSSim.roadPath(G, from.v, u, 0);
      if (!p) return;
      if (p.length < bestLen || (p.length === bestLen && u < best)) { bestLen = p.length; best = u; }
    });
    return best;
  };
...
H.run("farmstead-transport", async (t) => {
  ...
  /* ══════════ CONSTRUCTION FELLS THE WOOD (playtest §3, plan §14.13) ═══════
   * A dense stand used to be able to deadlock a settlement: trees refused
   * roads, flags AND buildings. Now the tree family is CLEARED by whatever
   * lands on it (the original let roads run straight through standing trees;
   * flags and buildings on one are our deviation), while STONE still refuses,
   * because a pile is the stonecutter's whole economy. Nothing yields wood. */
  const wood = await page.evaluate(() => {
    const FS = window.__FS__, T = window.T, FSSim = FS.FSSim, FSMap = FS.FSMap, FSC = FS.FSC, O = FS.FSC.OBJ;
    T.fresh();
    const G = FS.G, map = G.map, cf = T.cflag();
    const out = {};
    // ---- a road cut through a deliberately planted stand ----
    const end = T.flagSpot(4, 9);
    const path = FSSim.roadPath(G, cf.v, end, 0);
    const planted = [];
    for (let i = 1; i + 1 < path.length; i++) {
      if (map.obj[path[i]] !== O.NONE) continue;
      map.obj[path[i]] = O.TREE4; map.objArg[path[i]] = 0;      // a wall of mature trees
      planted.push(path[i]);
    }
    out.planted = planted.length;
    out.stepLegal = planted.length ? FSMap.whyRoadStep(map, path[0], planted[0], 0, {}) : "no trees";
    const ef = FSSim.placeFlag(G, end, 0);
    const lum0 = FSSim.production(G, 0).lumber || 0;
    const inv0 = FSSim.invOf(G, 0).lumber || 0;
    /* count from HERE: the end flag may itself have landed on a tree (that is
     * the point of the feature) and would otherwise be counted as the road's */
    const evBase = G.events.length;
    const r = FSSim.buildRoad(G, cf.id, ef.id, path, 0);
    out.roadOk = r.ok; out.roadWhy = r.why || null;
    out.stillTreed = planted.filter((v) => map.obj[v] !== O.NONE).length;
    out.lumberGain = ((FSSim.production(G, 0).lumber || 0) - lum0) + ((FSSim.invOf(G, 0).lumber || 0) - inv0);
    out.clearedEv = G.events.slice(evBase).filter((e) => e.type === "woodCleared").length;
    out.clearedN = (G.events.slice(evBase).filter((e) => e.type === "woodCleared")[0] || {}).n || 0;
    out.felledEv = G.events.filter((e) => e.type === "treeFelled").length;

    // ---- a FLAG and a BUILDING placed straight onto a tree ----
    T.fresh();
    const G2 = FS.G, m2 = G2.map;
    let fv = -1, bv = -1;
    FSMap.forRadius(m2, T.castle().v, 9, (u, d) => {
      if (d < 3) return;
      if (fv < 0 && m2.obj[u] === O.NONE && !FSMap.whyFlag(m2, u, 0)) fv = u;
    });
    if (fv >= 0) { m2.obj[fv] = O.TREE3; m2.objArg[fv] = 0; }
    out.flagOnTreeWhy = fv >= 0 ? FSMap.whyFlag(m2, fv, 0) : "no spot";
    out.flagOk = fv >= 0 ? FSSim.placeFlag(G2, fv, 0).ok : false;
    out.flagCleared = fv >= 0 && m2.obj[fv] === O.NONE;

    FSMap.forRadius(m2, T.castle().v, 12, (u, d) => {
      if (d < 4 || bv >= 0) return;
      if (m2.obj[u] === O.NONE && !FSMap.canPlaceBuilding(m2, "hut", u, 0)) return;
      if (m2.obj[u] === O.NONE && FSMap.canPlaceBuilding(m2, "hut", u, 0)) bv = u;
    });
    if (bv >= 0) { m2.obj[bv] = O.TREE4; m2.objArg[bv] = 0; }
    out.bldOnTreeWhy = bv >= 0 ? FSMap.whyBuilding(m2, "hut", bv, 0) : "no spot";
    out.bldOk = bv >= 0 ? FSSim.build(G2, "hut", bv, 0).ok : false;
    out.bldCleared = bv >= 0 && m2.obj[bv] === O.NONE;

    // ---- STONE still refuses all three ----
    let sv = -1;
    FSMap.forRadius(m2, T.castle().v, 10, (u, d) => {
      if (d < 3 || sv >= 0) return;
      if (m2.obj[u] === O.NONE && !FSMap.whyFlag(m2, u, 0)) sv = u;
    });
    if (sv >= 0) FSMap.setStone(m2, sv, 3);
    out.stoneFlagWhy = sv >= 0 ? FSMap.whyFlag(m2, sv, 0) : "no spot";
    out.stoneBldWhy = sv >= 0 ? FSMap.whyBuilding(m2, "hut", sv, 0) : "no spot";
    let nb = -1;
    for (let d = 0; d < 6 && nb < 0; d++) { const u = FSMap.nbr(m2, sv, d); if (u >= 0) nb = u; }
    out.stoneRoadWhy = nb >= 0 ? FSMap.whyRoadStep(m2, nb, sv, 0, {}) : "no nbr";
    return out;
  });
  t.check("a road may be laid through a stand of mature trees", wood.planted > 0 && wood.stepLegal === null && wood.roadOk === true, wood);
  t.check("…and every tree on its path is felled by the construction", wood.stillTreed === 0, wood);
  t.check("…for no wood at all (clearing is not a harvest)", wood.lumberGain === 0 && wood.felledEv === 0, wood);
  t.check("…reported once per construction, not once per tree",
    wood.clearedEv === 1 && wood.clearedN === wood.planted, wood);
  t.check("a flag may stand where a tree stood, and fells it", wood.flagOnTreeWhy === null && wood.flagOk && wood.flagCleared, wood);
  t.check("a building may too", wood.bldOnTreeWhy === null && wood.bldOk && wood.bldCleared, wood);
  t.check("STONE still refuses a flag, a building AND a road step",
    /blocked/.test(wood.stoneFlagWhy || "") && /blocked/.test(wood.stoneBldWhy || "")
    && /blocked/.test(wood.stoneRoadWhy || ""), wood);
  ...
  const prio = await page.evaluate(() => {
    ...
    // drain: budget has to cover the whole round trip, so scale it to the road
    let drained = false;
    for (let i = 0; i < 4000; i++) { FS.ff(1); if (!car.carry && !nf.slots.length) { drained = true; break; } }
    ...
    return {
      forced, preempt, midState, drained,
  ...
  t.check("the forced-order delivery drained the flag", prio.drained === true, prio);
  ...
  /* ═══════════════ buildings are SOLID (playtest 2026-08-01) ══════════════
   * `map.bldAt` only ever marked a building's ANCHOR, so five sevenths of a
   * castle was open meadow to every pathfinder in the game and settlers walked
   * straight through the great hall. The body is marked now — door excluded,
   * because the door carries the flag every road and every delivery needs. */
  const solid = await page.evaluate(() => {
    const FS = window.__FS__, T = window.T, FSSim = FS.FSSim, FSMap = FS.FSMap, FSC = FS.FSC;
    T.fresh();
    const G = FS.G, map = G.map;
    const castle = T.castle(), door = FSMap.doorVertex(map, castle.v);
    const body = FSMap.footprintOf(map, "castle", castle.v);
    const marked = body.filter((v) => map.bldFoot[v] === castle.id).length;

    /* Walk from one side of the castle to the far side. The route may touch the
     * door (that vertex is a flag, and roads run over it) but nothing else the
     * castle stands on. */
    function far(dirA, dirB) {
      let a = castle.v, b = castle.v;
      for (let i = 0; i < 4; i++) { a = FSMap.nbr(map, a, dirA); b = FSMap.nbr(map, b, dirB); }
      return [a, b];
    }
    const pair = far("NW", "SE");
    const path = FSSim.offroadPath(G, pair[0], pair[1]) || [];
    const through = path.filter((v) => v !== door && (map.bldAt[v] || map.bldFoot[v])).length;

    // …and a serf whose errand IS the castle still walks in at the anchor
    const inbound = FSSim.offroadPath(G, pair[0], castle.v) || [];
    const entersCastle = inbound.length > 1 && inbound[inbound.length - 1] === castle.v;

    // a road may not be laid through the body either
    const bodyRoad = body.length
      ? FSMap.whyRoadStep(map, FSMap.nbr(map, body[0], "E"), body[0], 0, { endB: true }) : "no body";
    // …but the door is still road-legal from outside
    let doorOk = null;
    for (let d = 0; d < 6; d++) {
      const u = FSMap.nbr(map, door, d);
      if (u < 0 || u === castle.v || map.bldFoot[u]) continue;
      if (!FSMap.whyRoadStep(map, u, door, 0, { endB: true })) { doorOk = u; break; }
    }
    // a small building marks nothing but its own vertex
    const hv = T.pickSite("hut", 3, 10);
    const hut = hv >= 0 ? G.buildings[FSSim.build(G, "hut", hv, 0).id] : null;
    const hutFoot = hut ? FSMap.footprintOf(map, "hut", hut.v).length : -1;
    // …and demolishing gives the ground back
    let cleared = -1, blockedWhileBurning = false;
    if (hut) {
      const big = T.pickSite("sawmill", 3, 12, hv);
      if (big >= 0) {
        const b2 = G.buildings[FSSim.build(G, "sawmill", big, 0).id];
        const id2 = b2.id;
        const f2 = FSMap.footprintOf(map, "sawmill", big);
        const on = f2.filter((v) => map.bldFoot[v] === id2).length;
        FSSim.demolishBuilding(G, id2);
        // burning walls are still walls — the ground comes back when they fall
        blockedWhileBurning = f2.some((v) => map.bldFoot[v] === id2);
        for (let i = 0; i < FSC.BURN_T * 3 && G.buildings[id2]; i++) FS.ff(1);
        cleared = on > 0 && f2.every((v) => !map.bldFoot[v]) ? on : -on;
      }
    }
    return { body: body.length, marked, through, pathLen: path.length,
      entersCastle, bodyRoad, doorOk, hutFoot, cleared, blockedWhileBurning, door };
  });
  t.check("a large building's BODY is marked, its door is not",
    solid.body >= 3 && solid.marked === solid.body, solid);
  t.check("a walker crossing to the castle's far side never stands on it",
    solid.pathLen > 0 && solid.through === 0, solid);
  t.check("…while a serf whose errand IS the castle still walks in",
    solid.entersCastle === true, solid);
  t.check("a road may not be laid through the walls", /building/.test(solid.bodyRoad || ""), solid);
  t.check("…but the door flag is still reachable by road", solid.doorOk !== null, solid);
  t.check("a small building takes only its own vertex", solid.hutFoot === 0, solid);
  t.check("burning walls still block, and the ground comes back when they fall",
    solid.blockedWhileBurning === true && solid.cleared > 0, solid);
  ...
  const render = await page.evaluate(() => {
    ...
    return { info, draws: st.drawCalls, tris: st.tris, named, serfs: info.serfs,
      poolKeys: Object.keys(info.pools), road: R.roadVisual(), spr: R.spriteInfo() };
  });
  ...
  /* ROADS ARE GROUND, NOT AN OBJECT (2026-08-01): a land network is ONE painted
   * ground decal, and dynamicInfo().roads counts DRAW CALLS (decal 0/1 + plank
   * ribbon 0/1) — an all-land town costs exactly one and needs no ribbon. */
  t.check("roads render as one painted ground decal, no ribbon",
    render.info.roads === 1 && render.info.roadDecal === true && render.info.roadWater === false, render.info);
  t.check("…and that decal really carries triangles", render.road.decal && render.road.tris > 0, render.road);
  /* FORK B (2026-08-01): the workforce is baked sprites now — the whole cast is
   * ONE instanced layer per subject instead of one merged mesh per (job,
   * player). The property under test is unchanged in spirit: people are
   * instanced, never one draw call each. Whichever path is live has to satisfy
   * it, so both are accepted and the live one is named. */
  t.check("serfs render instanced — sprite layers, or one mesh per job+player",
    render.spr.active
      ? (render.spr.counts.serf > 0 && render.spr.calls <= 3)
      : render.poolKeys.some((k) => k.indexOf("serf:") === 0),
    { sprites: render.spr.active, spr: render.spr.counts, pools: render.poolKeys });
  ...
  const shotSetup = await page.evaluate(() => {
    ...
    /* Hold the frame on a tick with scaffolding up, goods waiting and serfs
     * mid-stride. RESTAGED 2026-08-02: the scan budget was 1200 ticks, tuned on
     * a 64-wide medium map. Medium is 90 wide now (2x the area), the staged town
     * is spread over more ground, and a fresh site's DIGGER and BUILDER have a
     * correspondingly longer walk out from the castle — measured, `building`
     * never reached 1 inside two game-minutes, while `walking` (13) and `goods`
     * (29) cleared their bars immediately. The composition is unchanged; only
     * the time allowed for it to assemble is. `best` is reported so the next
     * person who sees this fail knows WHICH of the three never arrived. */
    let hit = null;
    const best = { walking: 0, goods: 0, building: 0 };
    for (let i = 0; i < 4000; i++) {
      FS.ff(1);
      ...
      if (walking > best.walking) best.walking = walking;
      if (c.goods > best.goods) best.goods = c.goods;
      if (building > best.building) best.building = building;
    ...
    return { hit, hud, counts: FSSim.counts(G, 0), best: best };
  });
  t.check("composed a busy frame: sites building, goods waiting, serfs walking", !!shotSetup.hit, shotSetup.best);
