...
const HELPERS = function () {
  ...
  T.spotsNear = function (type, from, dMin, dMax, p, skip, opts) {
    ...
    FSMap.forRadius(map, from, dMax, (u, d) => {
      ...
      // `unowned`: an outpost "in the middle of nowhere" has to be on ground
      // nobody holds, or the thing it is supposed to be holding up is already
      // being held up by its owner's castle
      if (opts.unowned && map.owner[u] >= 0) return;
      /* Never stage on ground an ENEMY TEAM already holds. Ownership is only
       * lent for the terrain test, so a spot inside a rival border passed
       * happily — and then the conquest cascade burned the freshly planted
       * building down on the next recompute, which reads as the mechanic under
       * test failing. (Bit once the 2026-08-01 generation pass moved the
       * starts.) An ALLY's ground stays fair game: co-op borders touching
       * without anything burning is itself one of the things tested here. */
      const ow = map.owner[u];
      if (ow >= 0 && ow !== p && G.players[ow] && G.players[p]
...
H.run("farmstead-military", async (t) => {
  ...
  const cascade = await page.evaluate(() => {
    ...
    const ov = T.spotNear("hut", T.castle().v, FSC.CASTLE_RADIUS + 3, FSC.CASTLE_RADIUS + 8, 1, null, { unowned: true });
  ...
  const aiTight = await page.evaluate(() => {
    const FS = window.__FS__, T = window.T, FSC = FS.FSC, FSSim = FS.FSSim, FSMap = FS.FSMap;
    /* RE-SCANNED 2026-08-01 (third time — the threshold is a property of the
     * PLACEMENT RULES, so every change to them invalidates it). Now that a tree
     * is FELLED by the construction that lands on it rather than refusing it,
     * forested ground counts as buildable and every start opened up: the old
     * seed 22 went 17 → 84 plots. Re-measured over seeds 1..60 on medium, seed
     * 13 is the tightest board left (47 plots inside the opening claim) against
     * a MEDIAN of 132 — so the assertion stays COMPARATIVE (about a third of a
     * normal board) instead of keeping an absolute number that no longer means
     * anything. The behavioural checks under it are unchanged. */
    /* RE-SCANNED A FOURTH TIME 2026-08-02: the medium board doubled in area
     * (64 → 90 wide) and lost a third of its water, so seed 13 opened right up
     * (47 → 154 plots). Over seeds 1..60 on the new generator the tightest
     * board is seed 11 at 84 plots against a MEDIAN of 196 — still well under
     * half a normal start, which is what "cramped" has to mean for the planner
     * test below to be testing anything. The bar moves with the board because
     * it is a RATIO; the behavioural checks under it are unchanged. */
    FS.newGame({ size: "medium", seed: 11, ais: 1, speed: 0 });
    const G = FS.G;
    const start = FSSim.castleOf(G, 1).v;
  ...
  t.check("the cramped start really is cramped (under half a median board)",
    aiTight.plots0 <= 98 && aiTight.plots0 > 0, aiTight);
  ...
  /* ══════════════ AI DIFFICULTY (playtest 2026-08-01) ═════════════════════
   * A table over the personalities. Two things have to hold: it is GAME state
   * (so a lockstep co-op game cannot have the two machines running different
   * opponents), and it actually MEASURES — an easy rival must build less, hold
   * less and field less than a hard one on the same board. */
  const diffWire = await page.evaluate(() => {
    const FS = window.__FS__, FSC = FS.FSC, FSSim = FS.FSSim;
    const g1 = FS.newGame({ size: "small", seed: 999, ais: 1, speed: 0, aiPlan: false, difficulty: "hard" });
    const rode = g1.difficulty;
    const round = FSSim.deserialize(FSSim.serialize(g1)).difficulty;   // survives the wire
    const g2 = FS.newGame({ size: "small", seed: 999, ais: 1, speed: 0, aiPlan: false, difficulty: "nonsense" });
    const table = FSC.AI_DIFF;
    return {
      rode, round, fallback: g2.difficulty, def: FSC.AI_DIFF_DEFAULT,
      names: FSC.AI_DIFFS.slice(),
      ordered: table.easy.period > table.normal.period && table.normal.period > table.hard.period
        && table.easy.expand < table.normal.expand && table.normal.expand < table.hard.expand
        && table.easy.aggro > table.normal.aggro && table.normal.aggro > table.hard.aggro
        && table.easy.attackT > table.normal.attackT && table.normal.attackT > table.hard.attackT
        && table.easy.answer <= table.normal.answer && table.normal.answer <= table.hard.answer,
      normalIsNeutral: table.normal.period === 1 && table.normal.expand === 1 && table.normal.aggro === 1
        && table.normal.build === 1 && table.normal.tries === 1 && table.normal.share === 1,
    };
  });
  t.check("difficulty rides in G, not in a client setting", diffWire.rode === "hard", diffWire);
  t.check("…and survives serialize/deserialize (a co-op guest inherits the host's)",
    diffWire.round === "hard", diffWire);
  t.check("…an unknown value falls back to the default", diffWire.fallback === diffWire.def, diffWire);
  t.check("the table is monotonic in every lever", diffWire.ordered === true, diffWire);
  t.check("…and NORMAL is exactly today's behaviour (all multipliers 1)",
    diffWire.normalIsNeutral === true, diffWire);

  const diffRun = await page.evaluate(() => {
    const FS = window.__FS__, FSSim = FS.FSSim;
    function run(difficulty) {
      FS.newGame({ size: "medium", seed: 12345, ais: 1, speed: 0, difficulty });
      const G = FS.G;
      for (let m = 0; m < 12; m++) FS.ff(600);
      const c = FSSim.counts(G, 1);
      let mil = 0, knights = 0;
      for (const id in G.buildings) {
        const b = G.buildings[id];
        if (b.p !== 1 || b.state === "burn") continue;
        if (b.mil && b.type !== "castle") mil++;
        if (b.mil) knights += b.mil.knights.length;
      }
      return { bld: c.buildings, land: c.land, mil, knights };
    }
    return { easy: run("easy"), normal: run("normal"), hard: run("hard") };
  });
  console.log("   difficulty @12min: easy " + JSON.stringify(diffRun.easy)
    + " normal " + JSON.stringify(diffRun.normal) + " hard " + JSON.stringify(diffRun.hard));
  const mono = (k) => diffRun.easy[k] <= diffRun.normal[k] && diffRun.normal[k] <= diffRun.hard[k];
  t.check("easy ≤ normal ≤ hard on buildings raised", mono("bld"), diffRun);
  t.check("…on territory held", mono("land"), diffRun);
  t.check("…and on the army in the field", mono("mil") && mono("knights"), diffRun);
  t.check("…with a real gap between the ends, not noise",
    diffRun.hard.bld > diffRun.easy.bld && diffRun.hard.knights > diffRun.easy.knights, diffRun);

  // ════════════════════════════════ AI vs AI: 60 sim-minutes of war
  const war = await page.evaluate(() => {
    const FS = window.__FS__, T = window.T, FSC = FS.FSC, FSSim = FS.FSSim;
    /* The 2026-08-01 generation pass deliberately pushes castles apart (a
     * medium board now averages 37% of its width between starts, up from 24%),
     * and seed 12345 — which used to sit them 14-18 steps apart — no longer
     * produces a war at all. Seed 101 is the closest equivalent under the new
     * generator: three starts 14 apart, near enough that two live opponents
     * genuinely come to blows (measured 25 attacks / 115 duel rounds). */
    /* RE-SCANNED 2026-08-02 for the same reason a THIRD time: on the 90-wide
     * board seed 101 puts its two AIs out of each other's reach and they never
     * fight. Scanned seeds 1..40 for the closest AI pair on the new generator —
     * seed 16 at 20 steps is the tightest, which is inside a hut's influence
     * ring and is where two live opponents come to blows. */
    FS.newGame({ size: "medium", seed: 16, ais: 2, speed: 0 });
    const G = FS.G;
  ...
  const crossSiege = await page.evaluate(() => {
    ...
    for (const t0 of spots) {
      ...
      for (let i = 0; i < near.length; i++) for (let j = i + 1; j < near.length; j++)
        /* The two besieging columns have to REACH the fight together, so the
         * sources must be about as far from the target as each other — with a
         * free choice one column routinely arrived after the place had already
         * fallen and the run was thrown away as "not contested". */
        if (FSMap.dist(map, near[i], near[j]) >= 5
          && Math.abs(FSMap.dist(map, near[i], t0) - FSMap.dist(map, near[j], t0)) <= 1) {
          T0 = t0; S0 = near[i]; S2 = near[j]; break outer;
        }
    }
    ...
    if (capturedAt < 0 || !preArrived || !preArrived.p2.length || !preArrived.p0.length) {
      return { ok: false, why: "did not land on a genuinely contested capture this run", capturedAt, preArrived };
    }
    const b = G.buildings[victim.id];
    /* WHICH column won is a coin toss the map decides — it flipped to player 2
     * when the 2026-08-01 generator moved the start sites. The rule under test
     * is symmetric ("only the CAPTURING player's knights are absorbed"), so
     * read the loser off the result instead of assuming player 0 won. */
    const lost = b.p === 0 ? preArrived.p2 : preArrived.p0;
    const loserP = b.p === 0 ? 2 : 0;
    const garrisonEvs = T.ev("knightGarrison").filter((e) => e.bld === victim.id);
    const wrongOwnerAbsorbed = garrisonEvs.filter((e) => lost.indexOf(e.id) >= 0).length;
    const rivalUntouchedAtCapture = lost.every((id) => {
      const s = G.serfs[id];
      return !!s && s.p === loserP && s.atkTarget === victim.id;
    });
    ...
    for (let i = 0; i < 6000 && resolvedAt < 0; i++) {
      FS.ff(1);
      const stillSieging = FSSim.serfsOf(G, loserP).filter((s) => s.job === "knight" && s.atkTarget === victim.id);
  ...
  /* FORK B (2026-08-01): knights come off the baked sheets, so "two knight
   * pools" (one per rank/player) became "knights on the knight sprite layer".
   * Both paths have to satisfy the same property — men in armour are on screen
   * for this shot — so whichever is live is the one asserted. */
  t.check("a duel is staged at the flag with knights on screen",
    shotBattle.ok && (shotBattle.spr.active
      ? shotBattle.spr.counts.knight >= 2
      : shotBattle.knightPools.length >= 2), shotBattle);
