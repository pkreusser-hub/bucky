...
/* RE-BASELINED 2026-08-01. These are golden hashes, not invariants: they exist
 * so a LOOK change can never quietly move the simulation. The playtest batch of
 * 2026-08-01 changed the sim and the world on purpose — buildings became solid
 * to through-traffic, and the generator was retuned for spacing, fairness,
 * climbable mountains and open green — so every number below moved, and both
 * halves (sim hash AND map hash) were recomputed together. Each was verified
 * REPRODUCIBLE (same seed twice, same hash) before being written down; the
 * property under test is unchanged. */
/* RE-BASELINED AGAIN 2026-08-01 (playtest batch #2). Two deliberate sim changes
 * moved every number: the STONECUTTER stopped drawing a random disc sample (one
 * fewer FSC.rng draw per attempt, so the whole stream shifts), and CONSTRUCTION
 * now FELLS TREES, which edits `map.obj` during play and therefore moves the MAP
 * hash as well as the sim hash. World GENERATION is untouched — the map hash at
 * tick 0 is unchanged and the world suite's determinism checks still pass; these
 * are post-simulation hashes. Recomputed twice and verified identical before
 * being written down (scratchpad rebase.cjs, "REPRODUCIBLE: true"). */
/* RE-BASELINED AGAIN 2026-08-02 (playtest batch #3). Three deliberate changes
 * moved every number and none of them is drift:
 *   · WORLD GENERATION itself changed — the maps are twice the area (small 68 /
 *     medium 90 / large 136), there is far less water and a notch more mountain,
 *     and every start is now guaranteed the ore its economy needs, which draws
 *     from FSC.rng while it seeds. A different world is a different everything.
 *   · the guarantee pass makes its own rng draws, so the whole stream shifts.
 *   · G.cons (the consumption ledger behind the rate HUD) is new state; it is a
 *     monotonic counter nothing reads back, so it cannot affect the sim, but it
 *     rides in the serialized G.
 * Recomputed TWICE in two fresh pages and verified identical before being
 * written down (scratchpad rebase3.cjs, "REPRODUCIBLE: true").
 *
 * RE-BASELINED AGAIN 2026-08-02 (batch #4) for ONE reason, and it is a rule
 * change rather than drift: CONSTRUCTION RUNS 1.5x FASTER (FSC.SWING_TICKS
 * [77,51,51,77] → [51,34,34,51], a deliberate deviation — see farmstead-plan.md
 * deviation 15). Every site finishes sooner, so at any given tick the
 * settlement is further along, which moves the sim hash; and because finished
 * buildings claim ground and their crews fell trees on the way, it moves the
 * MAP hash too. Nothing else in this batch touches the sim — the art, the
 * sprite sheets, the construction STAGES and the speed rail are all render or
 * UI. Recomputed twice in two fresh pages, "REPRODUCIBLE: true" again. */
const SIM_BASELINE = [
  { size: "small", seed: 101, ais: 1, ticks: 4000, hash: 1112582078, map: 1959488677 },
  { size: "medium", seed: 4242, ais: 1, ticks: 6000, hash: 4274176365, map: 4123514393 },
  { size: "medium", seed: 909, ais: 1, ticks: 6000, hash: 3753916887, map: 113490310 },
  { size: "large", seed: 31337, ais: 3, ticks: 5000, hash: 2520171034, map: 3217253263 },
  { size: "medium", seed: 12345, ais: 2, ticks: 8000, hash: 570491726, map: 3062816118 },
];
...
H.run("farmstead-visual", async (t) => {
  ...
  SIM_BASELINE.forEach((c, i) => {
    t.check("sim + map hash match the recorded baseline — " + sim[i].k,
  ...
  /* ══════════ 2b. the SPRITE gait: the same clock, stepped ═══════════════════
   * FORK B (2026-08-01). The three checks above are the ones that matter and
   * they are MODE-INDEPENDENT by construction: they read `R.serfPose()`, which
   * is the visual state both paths pose from, so a walk is judged on the same
   * numbers whether the man is 3D or a sprite. (That is also why the film
   * strips are rendered in the shipping default — sprites — rather than in the
   * fallback: the sheets are what the player sees, the telemetry the strips are
   * judged on does not move, and the 3D bone gait keeps its own geometry
   * assertions in the visuals suite's fallback sections.)
   *
   * What IS new in sprite mode is the frame-step signature: a continuous bone
   * swing becomes 8 baked frames chosen off that same phase. So: the drawn
   * frame must be exactly the one the phase asks for, it must actually step
   * while he walks, and it must freeze when the world does. */
  const sprGait = await page.evaluate(() => {
    const FS = window.__FS__, R = FS.FSRender, P = window.__P__;
    if (!R.spritesActive || !R.spritesActive()) return { skip: true };
    P.fresh({ seed: 4242, ff: 2600 });
    ...
    for (let i = 0; i < 40; i++) {
      P.beat(0.04);
      const pose = R.serfPose(mover.id), drew = R.spritePose(mover.id);
      if (!pose || !drew) continue;
      /* pass the serf ID: since 2026-08-02 the IDLE pose is a block of
       * variants keyed on the unit, so the resolver needs to know WHICH man */
      const want = R.spriteResolve({ kind: "serf", id: mover.id, pose: drew.pose, phase: pose.phase, yaw: pose.yaw });
      rows.push({ pose: drew.pose, k: drew.k, want: want.k, az: drew.az, wantAz: want.az, cell: drew.cell, wantCell: want.cell });
    }
    const walking = rows.filter((r) => r.pose === "walk");
    const mismatch = rows.filter((r) => r.k !== r.want || r.az !== r.wantAz || r.cell !== r.wantCell).length;
    const distinct = new Set(walking.map((r) => r.k)).size;
    /* …now pause and keep rendering: the frame must STOP ANIMATING.
     * RESTAGED 2026-08-01. This used to demand the post-pause cell equal the
     * pre-pause one, which quietly assumed the serf would still count as MOVING
     * after the clock stops. He often does not: with the sim paused his `frac`
     * never advances, the visual layer sees no motion and he settles from the
     * walk rows onto the IDLE frame — a man standing still in a stopped world,
     * which is right. The property under test is "it does not keep animating",
     * so: sample every frame after the pause and require the tail to be one
     * unchanging cell. That is strictly stronger than the old equality — a cell
     * that cycled would fail it — and it does not care whether the settle takes
     * one frame or five. */
    FS.setSpeed(0);
    const before = R.spritePose(mover.id);
    const paused = [];
    for (let i = 0; i < 12; i++) { P.beat(0.04); const p = R.spritePose(mover.id); if (p) paused.push(p.cell); }
    const tail = paused.slice(-6);
    R.setSpriteTrace(false);
    return { ok: true, n: rows.length, walking: walking.length, mismatch, distinct,
      before: before && { k: before.k, az: before.az, cell: before.cell },
      paused: paused, tail: tail,
      frozen: tail.length >= 6 && tail.every((c) => c === tail[0]) };
  });
  if (sprGait.skip) {
    t.check("(sprites off — the frame-step gait checks do not apply)", true, sprGait);
  } else {
    t.check("a walking serf really is drawn from the walk rows", sprGait.ok && sprGait.walking >= 8, sprGait);
    t.check("…every drawn cell is exactly the one his own phase and facing ask for",
      sprGait.ok && sprGait.mismatch === 0, sprGait);
    t.check("…and the gait visibly steps through the cycle, not one frozen frame",
      sprGait.ok && sprGait.distinct >= 3, sprGait);
    t.check("a paused world stops the sprite animating (the frame settles and holds)",
      sprGait.ok && sprGait.frozen, sprGait);
  }

  // ══════════════════════════════════════════ 3. the doorway
  const door = await page.evaluate(() => {
    const FS = window.__FS__, R = FS.FSRender, P = window.__P__;
    /* warm up further than before (2200): a medium map is 90 wide now, so at
     * 2200 ticks the settlement has carriers walking but nothing GARRISONED —
     * and a carrier absorbed into a warehouse stops existing partway down his
     * own fade, leaving no end of fade to read on him. */
    P.fresh({ seed: 4242, ff: 7000 });
    /* RESTAGED 2026-08-02. This used to latch onto the FIRST serf caught
     * mid-fade and read his appear twelve 0.04 s beats later. Two ways that
     * breaks on the new world, both seen: he can finish the fade and step back
     * OUT inside the window (appear 0.64 → 0.45 → 1, read as "never
     * disappeared"), and a carrier walking into a WAREHOUSE is absorbed
     * outright — he stops existing partway down and there is no end of fade to
     * read. The property is about the fade itself, so it is measured over the
     * whole settlement: every serf seen inside a building contributes his
     * appear, `frames` counts the partial values (it is a fade, not a blink)
     * and `ended` is the deepest value the fade actually reached. */
    const seen = [];
    let low = 1, sawInside = 0;
    for (let i = 0; i < 4000; i++) {
      ...
      for (const id in FS.G.serfs) {
        ...
        if (!p) continue;
        sawInside++;
        if (p.appear < 1) seen.push(p.appear);
        if (p.appear < low) low = p.appear;
      }
      if (low <= 0.02 && seen.length >= 4) break;
    }
    if (!sawInside) return { ok: false };
    return { ok: true, seen: seen.slice(0, 14),
      frames: seen.filter((v) => v > 0.02 && v < 0.98).length, ended: low };
  });
  ...
  const budget = await page.evaluate(() => {
    ...
    return { draws: R.stats().drawCalls, tris: R.stats().tris, fxMs: +fxMs.toFixed(2),
      ...
      legPools: Object.keys(d.pools).filter((k) => /^(serf|knight)leg/.test(k)).length,
      castOn: FS.FSModels.castOn(), spr: R.spriteInfo(),
  ...
  /* The villager's legs were sculpted per side, so each body kind needs a pool
   * each way; the symmetric minifig boot answers both from one. Either way the
   * cost is per WORKFORCE, not per serf, which is the property under test. */
  /* A CEILING, not an exact count: a pool only exists once something has drawn
   * from it, and whether a knight happens to be outside his walls in this
   * particular frame is not the property under test. The property is that the
   * PEOPLE cost per WORKFORCE — two pools for the symmetric minifig boot (serf
   * + knight), four for the villager's per-side sculpts — never per serf.
   *
   * FORK B (2026-08-01): under the sprite default there are no leg pools at all
   * — the legs are baked into the body cell — so the same property is read off
   * the sprite layers instead: three draw calls for every man on the map, legs,
   * hats and tools included. The 3D form still has to hold whenever the
   * fallback is what draws (see the visuals suite's §6c/§6e). */
  t.check("the whole workforce's legs cost a fixed handful of draw calls",
    budget.spr.active
      ? (budget.spr.calls >= 1 && budget.spr.calls <= 3 && budget.legPools === 0)
      : (budget.legPools >= 1 && budget.legPools <= (budget.castOn ? 4 : 2)), budget);
  ...
  const audio = await page.evaluate(() => {
    ...
    /* RESTAGED 2026-08-02: warm up FOUR times as long. A medium map is 90 wide
     * now (twice the area), the settlement takes correspondingly longer to
     * spread out, and at tick 1200 the 400-tick window that follows contained
     * no cue-bearing events at all — the one-for-one check was passing
     * vacuously and its own guard ("did the window contain anything?") caught
     * it, which is exactly what that guard is for. */
    P.fresh({ seed: 4242, ais: 1, speed: 1, ff: 5000 });
    const G = FS.G;
