...
async function main() {
  ...
  await page.goto(BASE + "/hayhem2.html", { waitUntil: "networkidle0", timeout: 90000 });
  await page.waitForFunction(() => !!window.__HAYHEM2__, { timeout: 30000 });
  ...
  const cm0 = await page.evaluate(() => window.__HAYHEM2__.camMode);
  check(cm0 === "overview" || cm0 === "fly", "match starts in overview (" + cm0 + ")");
  await page.waitForFunction(() => window.__HAYHEM2__.camMode === "aim", { timeout: 9000 });
  ...
  console.log("\n[2] pond / splash");
  const wi = await page.evaluate(() => window.__HAYHEM2__.waterInfo());
  ...
  await page.evaluate((b) => window.__HAYHEM2__.fireAt(b.pw, b.dir[0], b.dir[1], b.dir[2]), splash.best);
  await page.waitForFunction(() => window.__HAYHEM2__.G.lastImpact != null, { timeout: 15000 });
  ...
  const hitMoved = await page.evaluate((h) => {
    const tp = window.__HAYHEM2__.critters[h.targetId].bodies.torso.position;
  ...
  console.log("\n[5] turn system + AI");
  await page.evaluate(() => window.__HAYHEM2__.startMatch());
  await waitPhase(page, "aim");
  ...
  await waitResolve(page, 25000);
  const afterWest = await page.evaluate(() => ({ team: window.__HAYHEM2__.activeTeam, phase: window.__HAYHEM2__.phase }));
  check(afterWest.phase === "aim" && afterWest.team === 1, "after west shot → east's turn");
  // AI fires
  const aiFired = await page.evaluate(() => window.__HAYHEM2__.aiFireNow());
  ...
  await page.waitForFunction(() => {
    const H = window.__HAYHEM2__, pv = window.__pvStub, fl = H.G.flightLog;
  ...
  const previewMatch = await page.evaluate(() => {
    const H = window.__HAYHEM2__, pv = window.__pvStub, flight = H.G.flightLog.slice();
    let maxErr = 0;
    for (const a of pv) {
      let m = Infinity;
  ...
  await page.evaluate(() => window.__HAYHEM2__.startMatch());
  await waitPhase(page, "aim");
  const dunk1 = await page.evaluate(() => window.__HAYHEM2__.dunk(4));
  check(dunk1, "dunked east unit 4 into pond");
  await page.waitForFunction(() => window.__HAYHEM2__.G.units[4].out, { timeout: 8000 });
  const wout = await page.evaluate(() => ({ out: window.__HAYHEM2__.G.units[4].out, outs: window.__HAYHEM2__.G.outs[1], text: document.getElementById("eastVal").textContent }));
  ...
  await page.evaluate(() => {
    ...
    window.__HAYHEM2__.startMatch();
    window.__HAYHEM2__.forceCamMode("overview", true);
  });
  await waitPhase(page, "aim");
  await sleep(150);
  ...
  console.log("\n[9] desktop framing 1280×720 (overview)");
  await page.setViewport({ width: 1280, height: 720, deviceScaleFactor: 1 });
  await page.evaluate(() => { window.dispatchEvent(new Event("resize")); window.__HAYHEM2__.startMatch(); });
  await waitPhase(page, "aim");
  await sleep(150);
  ...
  async function fireBankShot(ammoType) {
    ...
    await page.waitForFunction(() => window.__HAYHEM2__.G.projectiles.length === 0, { timeout: 20000 });
    const carved = await page.evaluate((b) => b - window.__HAYHEM2__.voxelCount(), found.before);
    await waitResolve(page, 25000);
    return carved;
  }
  const selMelon = await page.evaluate(() => window.__HAYHEM2__.setAmmo("melon"));
  ...
  const melonCount = await page.evaluate(() => window.__HAYHEM2__.ammoCounts()[0].melon);
  ...
  await page.waitForFunction(() => window.__HAYHEM2__.G.projectiles.length === 3, { timeout: 10000 });
  check(true, "egg split at apex into 3 projectiles (G.projectiles hit 3 mid-flight)");
  await waitResolve(page, 25000);
  const eggAfter = await page.evaluate(() => ({
    projs: window.__HAYHEM2__.G.projectiles.length, phase: window.__HAYHEM2__.phase,
    egg: window.__HAYHEM2__.ammoCounts()[0].egg
  }));
  ...
  await page.evaluate(() => window.__HAYHEM2__.startMatch());
  const cmStart = await page.evaluate(() => window.__HAYHEM2__.camMode);
  ...
  await page.waitForFunction(() => window.__HAYHEM2__.camMode === "overview", { timeout: 15000 });
  const tOv = Date.now();
  await page.waitForFunction(() => window.__HAYHEM2__.camMode !== "overview", { timeout: 15000 });
  ...
  await page.evaluate(() => window.__HAYHEM2__.startMatch());
  await page.waitForFunction(() => { const H = window.__HAYHEM2__; return H.camMode === "aim" && H.activeTeam === 0; }, { timeout: 9000 });
  await sleep(140);
