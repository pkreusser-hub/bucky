...
H.run("farmstead-polish", async (t) => {
  ...
  const reload1 = await page.goto(t.BASE + "/castlekruzer.html?r=" + Date.now(), { waitUntil: "domcontentloaded" });
  ...
  await page.goto(t.BASE + "/castlekruzer.html?r=" + Date.now(), { waitUntil: "domcontentloaded" });
  await page.waitForFunction(() => !!window.FSAudio, { timeout: 20000 });
  ...
  await mob.goto(t.BASE + "/castlekruzer.html", { waitUntil: "domcontentloaded" });
  ...
  /* RESTAGED 2026-08-01 (third time for this check, and back where it started):
   * the 2-finger twist rotated the camera, then Fork B's yaw lock made the same
   * gesture a pure pan, and now the yaw is free again so it rotates once more.
   * The gesture drags its centroid as well as turning, so BOTH halves are
   * asserted — a twist that only panned would pass a yaw-only check by luck. */
  t.check("touch: 2-finger twist turns the world again", Math.abs(touchRotate.after.yaw - touchRotate.before.yaw) > 0.02, touchRotate);
  t.check("…and the gesture still pans", Math.abs(touchRotate.after.tx - touchRotate.before.tx)
    + Math.abs(touchRotate.after.tz - touchRotate.before.tz) > 0.5, touchRotate);
  ...
  await perfPage.goto(t.BASE + "/castlekruzer.html", { waitUntil: "domcontentloaded" });
