...
H.run("farmstead-economy", async (t) => {
  ...
  const sweep = await page.evaluate(() => {
    ...
    // (fish regrowth/migration is proven in its own controlled block below —
    // on CONNECTED water a "dead zone" can always be re-seeded by multi-hop
    // drift, so geometric isolation is not a valid way to test the >0 guard)
  ...
  const fishGuard = await page.evaluate(() => {
    ...
    const G = FS.G, map = G.map, N = map.W * map.H;
    const pass = T.sweepPass();
    for (let v = 0; v < N; v++) map.fish[v] = 0;
    FS.ff(pass * 2);
    let deadTotal = 0;
    for (let v = 0; v < N; v++) deadTotal += map.fish[v];
    // seed ONE shoal; drift travels at most 1 hop per pass, so after 2 passes
    // fish may exist only within 2 hops of the seed (regrowth needs fish>0)
    let seed = -1;
    /* The shoal must have somewhere to DRIFT to, and drift only ever goes in
     * FSC.FISH_MIGRATE_DIRS — "any water neighbour" was picking tiles whose
     * only opening lay on one of the two dead axes, so the shoal sat still and
     * the check failed on the water rather than on the migration. Two open
     * directions (a coin flip per visit) over six passes leaves a ~1.6% chance
     * of no drift at all; two passes with one open direction was a coin flip. */
    FS.FSMap.forRadius(map, T.castle().v, 20, (u) => {
      if (seed >= 0 || map.terr[u] !== FSC.TERR.WATER) return;
      let open = 0;
      for (let k = 0; k < FSC.FISH_MIGRATE_DIRS.length; k++) {
        const n = FS.FSMap.nbr(map, u, FSC.FISH_MIGRATE_DIRS[k]);
        if (n >= 0 && map.terr[n] === FSC.TERR.WATER) open++;
      }
      if (open >= 2) seed = u;
    });
    map.fish[seed] = 6;
    FS.ff(pass * 6);
    // drift is 1 hop per visit, but the stride can visit a just-fed tile later
    // in the SAME pass and chain the hop — allow a generous envelope
    const near = new Set();
    FS.FSMap.forRadius(map, seed, 10, (u) => near.add(u));
    let nearTotal = 0, farTotal = 0, spread = 0, overCap = 0;
    for (let v = 0; v < N; v++) {
      if (map.fish[v] > FSC.FISH_CAP) overCap++;
      if (!map.fish[v]) continue;
      if (near.has(v)) { nearTotal += map.fish[v]; if (v !== seed) spread++; }
      else farTotal += map.fish[v];
    }
  ...
  /* ══════════ the stonecutter takes the NEAREST rock (playtest §4, plan §14.14)
   * The one profession that does NOT use the original's single random disc
   * sample. Staged as the complaint describes it: one hut with a near group and
   * a far group both in range. He must work the near one out FIRST and only
   * then walk to the far one — and he must do it the same way on every client,
   * so the same world is run twice and the sequence compared. */
  const near = await page.evaluate(() => {
    const FS = window.__FS__, T = window.T, FSC = FS.FSC, FSMap = FS.FSMap;
    function run() {
      T.fresh();
      const G = FS.G, map = G.map;
      const sc = T.add("stonecutter", { minD: 3, maxD: 16 });
      if (!sc) return null;
      const R = FSC.BLD.stonecutter.radius;
      // wipe every pile in range, then plant two groups at known distances
      FSMap.forRadius(map, sc.v, R + 2, (u) => {
        if (FSMap.isStone(map.obj[u])) { map.obj[u] = FSC.OBJ.NONE; map.objArg[u] = 0; }
      });
      const nearV = [], farV = [];
      FSMap.forRadius(map, sc.v, R, (u, d) => {
        if (map.obj[u] !== FSC.OBJ.NONE || map.flagAt[u] || FS.FSSim.bldBlocks(G, u)) return;
        if (!FSMap.walkable(map.terr[u])) return;
        /* RESTAGED 2026-08-02: …and the cutter has to be able to WALK there.
         * `walkable` is a terrain-class test; it says nothing about the slope
         * between here and there, and the 2026-08-02 world has a good deal more
         * mountain in it. On the new medium map this staging planted its "near"
         * pair three steps away up a shoulder the worker could not climb, so he
         * legitimately went to the far group — the test was measuring its own
         * staging, not the picker. */
        if (!FS.FSSim.offroadPath(G, sc.v, u, 0)) return;
        if (d >= 2 && d <= 3 && nearV.length < 2) nearV.push(u);
        else if (d >= 6 && d <= R && farV.length < 2) farV.push(u);
      });
      if (nearV.length < 2 || farV.length < 2) return null;
      nearV.concat(farV).forEach((u) => FSMap.setStone(map, u, 2));
      /* RESTAGED 2026-08-02: read the WHOLE event sequence, not one sample per
       * trip. This used to poll G.events once per estimated round trip and take
       * the most recent stoneCut, which quietly assumes the estimate stays in
       * phase with the real trip. On the 2026-08-02 world the near pair sits on
       * different ground from the far pair, the two trips take visibly different
       * numbers of ticks, and the poll skipped straight past the near cuts: the
       * cutter really did work 5650 then 5651 twice before touching the far
       * group, and the test still reported "far first". Draining by event
       * watermark cannot miss a cut and cannot be thrown by the buffer cap. */
      const order = [];
      let seenT = -1;
      const drain = () => {
        const evs = FS.G.events;
        for (let k = 0; k < evs.length; k++) {
          const e = evs[k];
          if (e.t <= seenT || e.type !== "stoneCut") continue;
          if (!order.length || order[order.length - 1] !== e.v) order.push(e.v);
        }
        if (evs.length) seenT = evs[evs.length - 1].t;
      };
      const total = 8 * (nearV.length + farV.length);
      for (let i = 0; i < total; i++) { FS.ff(T.tripTicks("stonecutter")); drain(); }
      return { nearV, farV, order, hut: sc.v,
        cutNear: order.filter((v) => nearV.indexOf(v) >= 0).length,
        cutFar: order.filter((v) => farV.indexOf(v) >= 0).length };
    }
    const a = run(), b = run();
    if (!a || !b) return { ok: false };
    // the FIRST far cut may not come before the LAST near cut
    const firstFar = a.order.findIndex((v) => a.farV.indexOf(v) >= 0);
    const lastNear = a.order.reduce((acc, v, i) => (a.nearV.indexOf(v) >= 0 ? i : acc), -1);
    return { ok: true, firstFar, lastNear, cutNear: a.cutNear, cutFar: a.cutFar,
      same: a.order.join() === b.order.join(), n: a.order.length,
      _dbg: { hut: a.hut, nearV: a.nearV, farV: a.farV, order: a.order } };
  });
  t.check("the stonecutter works the NEAR group before the far one",
    near.ok && near.cutNear > 0 && (near.firstFar < 0 || near.firstFar > near.lastNear), near);
  t.check("…then moves on to the next-nearest once it is worked out",
    near.ok && near.cutFar > 0, near);
  t.check("…and the choice is deterministic (same world twice, same sequence)",
    near.ok && near.same, near);
  ...
  const meat = await page.evaluate(() => {
    ...
    /* Breeding is a lottery (FSC.PIG_ROLLS rolls at ~9% each), so the run has
     * to be long enough that "the herd never grew" means the mechanic is
     * broken rather than that the dice were cold — 14 cycles was inside the
     * noise and started failing the moment the world generator's rng stream
     * moved under it. */
    FS.ff(FSC.BLD.pigfarm.cycleT * 30);
    const prod = T.prod();
  ...
  const works = await page.evaluate(() => {
    ...
    T.fresh();
    const G = FS.G;
    /* sep 3 → 4 (2026-08-01, tree-clearing): T.pickSite is greedy-NEAREST, and
     * now that a forested vertex is a legal plot (construction fells the tree)
     * the nearest legal ground is much closer in. Five LARGE workshops — each a
     * SOLID 7-vertex footprint — at 3 lattice steps apart left no lane for a
     * road and every one of them failed to connect. Four steps is one clear
     * vertex between bodies, which is what a player would leave. Staging only:
     * the checks below are about the RECIPES. */
    const q = T.addMany(["smelter", "goldsmelter", "toolmaker", "weaponsmith", "boatwright"],
      { minD: 3, maxD: 16, sep: 4 });
  ...
  const water = await page.evaluate(() => {
    ...
    /* Re-plot the crossing AFTER wiring the near shore into the network: the
     * land road just laid can share its first edge with the water path, and
     * buildRoad rightly refuses to lay a road over one ("already a road").
     * roadPath already avoids used edges, so asking again is the whole fix. */
    const p2 = FS.FSSim.roadPath(G, A, B, 0, { water: true, maxLen: 80 });
    if (p2) path = p2;
  ...
  const shot1 = await page.evaluate(() => {
    ...
    T.fresh();
    const G = FS.G;
    // sep 4 for the same reason as the workshop quarter above — solid footprints
    // need a road lane once forest plots became legal (2026-08-01)
    T.addMany(["lumberjack", "sawmill", "forester", "farm", "mill", "bakery", "stonecutter",
      "pigfarm", "butcher", "toolmaker", "smelter", "stock"], { minD: 3, maxD: 18, sep: 4 });
    FS.ff(26000);
    // hold the frame on a busy moment: goods on flags, workers outside, machines running
    /* This is a SHOT COMPOSER, not a behaviour check: it waits for a frame with
     * visible work in it. Widened 900 → 3600 ticks and loosened from "two goods
     * ON A FLAG right now" to "somebody is out working AND something is either
     * being made or being carried" (2026-08-01) — a settled network can run a
     * whole minute with its flags momentarily empty, which said nothing about
     * the economy and everything about when the sampler happened to look. */
    let hit = null;
    for (let i = 0; i < 3600 && !hit; i++) {
      FS.ff(1);
      const serfs = FS.FSSim.serfsOf(G, 0);
      const outside = serfs.filter((s) => /goWork|doWork|backWork/.test(s.state)).length;
      const carrying = serfs.filter((s) => s.carry).length;
      const working = Object.keys(G.buildings).filter((id) => G.buildings[id].working).length;
      const goods = FS.FSSim.counts(G, 0).goods;
      if (outside >= 1 && (working >= 1 || goods >= 1 || carrying >= 1)) {
        hit = { outside, working, goods, carrying, tick: G.tick };
      }
    }
