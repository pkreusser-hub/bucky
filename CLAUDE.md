...
# ðŸ° FARMSTEAD â€” mechanics-faithful Settlers-1 clone in 3D (2026-08-01, branch claude/settlers-3d-clone-rdg6wl, draft PR #2)

**LAUNCH NAME: CASTLE KRUZER (2026-08-02).** Player-facing rename only â€” page is now
...
â€” same precedent as Huey keeping his goose files. Added to games.html's `GAMES` + index.html's
`PLAY_GAMES` (ðŸ°, `castlekruzer.html`) and games.html's `LOBBY_TEXT` ("kingdom is open!" /
"mid-siege!"). Sprite-bake source GLBs (`dwarf.glb`/`knight.glb`) were already copied into
`assets/farmstead/cast/dwarfknight/src/` in the 2026-08-01 session â€” the production bake/rig
pipeline (`_fs_bake_sprites.cjs` â†’ `_fs_dk_rig.py`) has no Downloads dependency; only the
abandoned, explicitly-"never shipped" `sprites-test` exploration fork still names Downloads,
in provenance comments only. Full 9-suite battery re-verified GREEN post-rename: world 84 Â·
transport 138 Â· economy 107 Â· military 124 Â· mp 127 Â· ui 149 Â· visuals 187 Â· visual 60 Â·
...
+ DOSBox observation of the user's own copy) â€” never code/art/audio from it. NEVER commit
any user-uploaded game asset; the custom-music loader keeps user audio in IndexedDB only.
- **Determinism (lockstep law)**: sim files use FSC.rng ONLY â€” no Math.random/transcendentals/
  ...
  guest lead â‰¤ lastConfirmed + CMD_DELAY_MPÃ—max(1,speed) âˆ’ 1). Co-op modes: shared kingdom
...
- **Original-exact systems**: costs, two 26-item priority lists, slope walk table
  ...
  exhaustion, fish 0..7 regrow(>0-guarded)+migration, sweep (SAPLING: ONE 25% roll â†’ mature),
  ...
  farmstead-plan.md Â§14 (incl. Â§14.10 flag-removal goods lost, Â§14.11 pickup aging).
- **Tests**: `tools/_fs_harness.cjs` + NINE suites `tools/_verify-farmstead-{world,transport,
  economy,military,mp,ui,visuals,polish,visual}.cjs` (~830 checks post-review). UI-suite 2Ã—/4Ã—
  speed-ratio checks are LOAD-SENSITIVE â€” run the board on an idle box. buildingDetail counts
  indexed geo by index.count/3.
- **ADVERSARIAL REVIEW 2026-08-01**: 6 finders + 6 paired refuting verifiers (Workflow) â†’ 30
  confirmed findings, ALL fixed in 4 waves (sim-core 6a6c52e Â· military 454575e Â· netcode
  088661d Â· UX/integration follow-up). Highlights: tickRetry in-flight booking leak (zeroing
...
- **Castle**: user's Tripo GLB was tried and REPLACED by an original procedural model in its
  ...
  lift â€” the shared warm BLD_WALL lift creams out grey stone; motte frustums share seam rings
  exactly or sky grins through). GLB + GLTFLoader removed (git history only).
- **Not linked** from games.html/index PLAY_GAMES â€” happens at user-approved merge, never
  push to main without preview approval (auto-deploy).
- **PICKED UP LOCALLY 2026-08-01** (cloud â†’ local): files extracted from
  `origin/claude/roads-wip-backup` â€” that ref = branch tip 1a85619 PLUS the cloud session's
  uncommitted ROADS WIP snapshot (fs-models.js/fs-render.js +501/âˆ’22, road visuals mid-build,
  saved stash-style "pre-teleport") â€” onto the local working tree as UNTRACKED WIP on
...
- ðŸ›¤ **ROADS ARE GROUND, NOT AN OBJECT â€” FINISHED 2026-08-01** (user playtest: "too blocky and
  perfectâ€¦ should be a texture applied to the ground instead of an object"). The cloud WIP had
  ...
  map-space canvas (Chaikin corner-cutting kills the 60Â°/120Â° lattice kinks, `wobbleSeries`
  ...
  one lattice ring wide, lifted `ROAD_DECAL_LIFT = ROAD_LIFT*0.5` with polygonOffset âˆ’2.
  Water spans keep the plank ribbon (`roads:water`) â€” there is no ground under a causeway to
  ...
  (`rebuildRoads(false)`, sheet reused â€” a digger marks dirtyV every swing and a 2048px repaint
  ...
  **THREE BUGS FOUND AND FIXED in this finishing pass** (all in the "does it read as ground or
  as a sticker" family â€” the WIP got the paint right and the integration wrong):
  (1) **STALE TERRAIN NORMALS.** The decal copies `terrainMesh.geometry.attributes.normal` so it
  lights like the ground, but `frame()` ran `computeVertexNormals()` AFTER `syncDynamic()` â€” so
  ...
  outside syncDynamic. Measured: worst normal mismatch after an edit 0.196 â†’ 0.
  (2) **WRONG AMBIENT LIFT.** The decal inherited the ribbon's `emissiveOf: COL.ROAD,
  emissiveK: 0.22` (= +34/255 red). The terrain it lies ON uses `0x6d8b5c Ã— TERR_EMISSIVE_K
  0.14` (= +15). More than double the lift of its own ground, and because emissive is a FLAT
  add it raised the dark ruts and packed core by exactly as much as the pale verge â€” crushing
  ...
  wedge lying on the grass at every road end â€” the exact "blocky and perfect" read the rework
  ...
  â†’ the whole network ~20% brighter than the road it replaced. `ROAD_GRAIN = 0.82` folds that
  factor back in ONCE over `ROADPAINT_RAW` so every coat/rut/blot keeps its authored
  relationship. Measured median rendered road colour (203,181,128) â†’ (156,146,103) against
  grass (115,151,90).
  **Hunted and clean** (all asserted, not eyeballed): no z-fighting at 18Â° grazing pitch, no
  ...
  Junction blobs are NOT misaligned â€” Chaikin leaves a polyline's first/last point untouched
  ...
  **KNOWN LIMIT** (documented, not a bug): the sheet frames the NETWORK, so px/world-unit falls
  as the town spreads â€” measured 86 px/u on a young settlement, 50 at 57 roads, ~16 worst case
  if a network ever spanned a whole medium map (painted half-width 40px â†’ 7px). `paintRoadStroke`
  ...
  **Suites** (`.roads` semantics changed, so assertions had to move with it): transport's "roads
  render as one merged ribbon" â†’ "one painted ground decal, no ribbon" + a decal-has-triangles
  check (122â†’123). visuals gained a 24-check section **2b** covering the whole feature â€”
  ...
  roads===2), demolition shrink/empty/no-geometry-leak (58â†’82). Sim-level road checks needed no
  changes. **TEST GOTCHAS**: two flags may never be adjacent ("flag too close"), so the shortest
  legal road over one steep step is flagâ€”stepâ€”flag (a 1-edge test road is impossible); a generic
  ...
  from `gl.readPixels`, not `page.screenshot` â€” the HUD is DOM over the canvas and fills a
  centred transect with panel grey.
  **Full sweep**: world 75/75 Â· transport 123/123 Â· economy 104/104 Â· visuals 82/82 Â· visual
  56/56 Â· military 115/115 Â· ui 120/120, 0 pageerrors. TWO PRE-EXISTING FAILURES, both proven
  ...
  reload â€” reproduced in isolation, the upload works fine WITHOUT the suite's two reloads, so
  the settings file-input handler is not re-wired on reload; audio area, untouched here) and
  mp 124/126 (the far-behind-guest "catching upâ€¦" veil never shows). The visual suite also
  failed on 0/10 film strips being on disk â€” those are review ARTEFACTS that never came across
  with the untracked WIP; regenerated with `node tools/_fs_filmstrip.cjs <subjectâ€¦>` â†’ 56/56.
  Shots: `shots/fs_roads_{network,grazing,slope,water,town}.png`.
- ðŸ§‘â€ðŸŒ¾ **THE VILLAGER â€” the people are a sculpt now (2026-08-01)**. Serfs AND knights render
  ...
  deleted â€” it is the fallback, and `FSModels.castOn()` is the only switch.
  **VARIANT = vertex-coloured, used RAW.** The textured variant was rejected on architecture,
  ...
  UV-unwrapped face). The vc cut IS that texture, sampled per vertex. âš  The obvious
  ...
  (0.690,0.486,0.333) = rgb8(176,124,85) â€” the baker wrote sRGB texels, not linear. Converting
  ...
  the screen unconverted too â€” raw IS the world's space. Only tuning applied: chroma Ã—1.18
  ...
  **LOADER = ~90 lines, no GLTFLoader.** farmstead lost its loader with the castle GLB and
  ...
  tight float attributes) â€” re-vendoring 40 KB for skins/Draco/cameras nothing uses is the
  ...
  **GARMENTS ARE DYED, NOT BUILT â€” two wrong answers first, both instructive.** (1) Primitives
  ...
  construction, still tore â€” an offset surface self-intersects at high curvature, so bare skin
  came back through the surcoat at the shoulder roll â€” and cost 3000 tris for one coat. The
  ...
  â‰¥ ~0.10 tall or the shipped 1498-tri cut goes threadbare â€” a triangle spans ~0.03 of him.
  Team reads three ways (shoulder YOKE for the overhead camera, waist BELT â€” the one height a
  band passes UNDER the hanging sleeves â€” and a chest baldric); job is the cap; knights get a
  full surcoat + mail coif + steel helm + greaves, rank still on plume/shield-rim/pips.
  **HIPS ARE PER BODY KIND** (`FSModels.hipOf`/`knightHipOf`): villager Â±0.1235/0.255/z0.028
  vs the minifig's Â±0.075, swing 0.34 rad (not 0.52) about a pivot 0.085 BELOW the joint. The
  ...
  **PERF â€” measured, and the fix is not what the brief assumed.** Large developed town, 76
  ...
  tris â†’ full 6000-tri villager 168 ms / 675k tris = **+58%**, over the +30% budget. The cause
  is not that 6000 is big, it is that it is wrong for the SCREEN SIZE â€” a 0.79-unit villager at
  the camera's usual 26-40 units runs a dozen triangles per pixel, and sub-pixel triangles cost
  on a GPU too (each shades a 2Ã—2 quad), so this is not a headless artefact. Decimating the
  ...
  **1498 / 271 / 270** (`tools/_villager_lod.py`, re-runnable from the vc GLBs at zero Tripo
  cost â€” vertex colours ride through the CORNER-domain attribute, mean COLOR_0 drift
  0.6898â†’0.6875, no re-bake and no need for the 100 MB fullres .blend). Re-measured: **107 ms /
  358k tris = +19%**. At `CAM.DIST_MIN` (8, the closest a player can legally get) the two cuts
  are indistinguishable â€” shots prove it. The full 6000 cut still ships behind
  ...
  **Suites**: visuals gained section 6b (17 checks: load/indexed/vertex-coloured, per-side legs,
  ...
  stride travel, pool count â€” plus a request-interception page where the .glb is BLOCKED, proving
  the load resolves false, the minifig takes over on its own hips with ONE leg pool per kind,
  and the settlement still fills with people, 0 page errors) â†’ **82â†’99**. Its `tris()` helper is
  now `FSModels.triCount` (index-aware â€” bodies ship INDEXED; de-indexing the villager would
  ...
  world 75 Â· transport 123 Â· economy 104 Â· visuals 99 Â· visual 56 Â· military 115 Â· ui 120 Â·
  polish 67 Â· mp 124/126 (the known far-behind-guest veil, untouched). Film strips regenerated
  (serf-walk-flat / serf-gait-closeup / serf-door / duel / knight-march; gait CV 0.107, maxJump
  1.02x, yaw 0 â€” no foot skate).
  **TEST GOTCHAS**: a bounding box is the WRONG way to measure stride â€” the leg's upper mass
  ...
  geometry call. The blocked-GLB page logs three `net::ERR_FAILED` console errors by design â€”
...
## ðŸŽž SPRITE-IMPOSTOR VIABILITY DEMO â€” exploration only, NOT shipped (2026-08-01)

User question: "what if we kept the 3D terrain and camera/map but we used sprites and had
that 3d rotation effect â€” gauge how viable that would be." Answered with a live page, not a
memo. **`farmstead-proto/sprite/spritedemo.html`** (frozen-experiment precedent: the whole
dir is self-contained â€” its own copies of fs-const/fs-map, the terrain+lighting+fog+camera
rig lifted verbatim out of fs-render, and its own vendored r128 GLTFLoader. The live
fs-models.js / fs-render.js / farmstead.html were neither edited nor runtime-imported â€”
...
[--shots]` â†’ 38/38, 0 pageerrors. 10 screenshots `shots/fs_sprite_*.png`.
...
pixels change by >12/255 (whole-frame mean delta ~2/255) â€” see `fs_sprite_sidebyside.png`,
...
the integration already ships (2,039 tris) puts 800 serfs at 1.6M tris â€” an ordinary frame.
...
orbit 360Â° (`CAM.ORBIT_YAW` on right/shift-drag, Q/E) with pitch 35â€“70Â°, so full azimuth
coverage is mandatory and pitch is the unsolved axis (bake 2-3 pitch rows if it ships) Â·
**16 angles is the setting** â€” per-frame change across one 22.5Â° bin is 6.5-7.1 vs the real
mesh's 6.7-7.0, i.e. statistically identical; 8 angles is 13.3-13.6, visibly popping Â· 128px
cells cover the whole zoom range (villager is 26px at DIST_START 34, 109px at DIST_MIN 8) â†’
2048Ã—896, 9.4 MB Â· a Î”-indexed atlas provably CANNOT carry a world-fixed sun (one frame
serves every facing/camera pair with the same difference), so the key light is baked
camera-relative; measured cost of the wrong sun side is 12.7/255 (~13.8%) Â· impostors are a
TRIANGLE win, not a draw-call win (mesh path is already instanced) Â· at 1000 units the blob
SHADOWS cost 10,000 tris, 5Ã— the sprites Â· **statics: don't** â€” after a ~110Â° orbit the barn
...
- `WebGLRenderer.setViewport/setScissor` take LOGICAL px and multiply by pixelRatio
  ...
  `setViewport(0,0,canvas.width,â€¦)` â€” canvas.width is already DEVICE px â€” doubles again and
...
- The impostor quad must be VIEW-ALIGNED (camera image plane), not cylindrical: the bake is
  ...
  and still reads as a card in a hole. And it needs a DEPTH BIAS toward the camera (~0.22 Ã—
  quad width) â€” an ortho bake contains pixels below the ground anchor (the near foot seen from
...
- Headless: use `--use-angle=swiftshader` (the house convention), NOT `--use-gl=swiftshader` â€”
...
- The villager's COLOR_0 is DISPLAY-referred, not scene-linear (measured mean 0.69/0.49/0.33
  ...
  white. The game's own integration reached the same answer (`cast.srgb = false`) â€” two
...
## ðŸŒ¾ FARMSTEAD PLAYTEST BATCH â€” ten fixes from one session at the wheel (2026-08-01)

User playtest, ten items. Every number below is MEASURED â€” 8 medium boards, 4 players,
...
**1-4 Â· WORLD GENERATION.** grass 26.1% â†’ **33.7%** (water 49.1 â†’ 40.7, swamp 5.9 â†’ 4.9,
desert 4.6 â†’ 3.6); castle separation 15.5 â†’ **23.5** steps = 24% â†’ **37%** of map width
(and the start-quality tier settles at 2 on every seed instead of drifting to 3-4);
mine-placeable mountain 52.4% â†’ **92.3%**; ore sitting on mine-placeable ground 1598 â†’
3430; per-start budget spread (maxâˆ’min over mean) wood **1.22 â†’ 0.25**, stone **1.08 â†’
0.34**, ore **2.07 â†’ 0.57**; buildable plots per start (144 starts over 3 sizes Ã— 12 seeds)
worst **1 â†’ 10**, mean 82 â†’ 125. Determinism re-proved: same seed twice = same mapHash
across every size and player count, and the full sim replays identically at 3000 ticks.
- **MINES were never a mine problem â€” they were a ROAD problem.** `whyBuilding` has no
  ...
  (`smoothLowland` deliberately left the high ground rugged). `MOUNT_H` 12 â†’ **5.2** plus a
  smoothing FLOOR `SMOOTH_W_MOUNT` 0.42 that applies at any altitude; `MOUNTAIN_Y`/`SNOW_Y`
  re-anchored (2.72 â†’ 2.30, 11.2 â†’ 5.6) so the mountain AREA â€” and therefore the ore â€” barely
  moves. Mean ruggedness (largest drop to any neighbour) 3.23 â†’ 1.11, i.e. under the road cap.
- **SEPARATION**: the ladder now weighs the farthest-point set at EVERY rung instead of only
  after the whole ladder failed â€” a candidate pool clustered on the best-scoring corner used
  to walk all the way down to the floor. Plus `SEP_FRAC[4]` 0.36 â†’ 0.46 and floors raised.
- **FAIRNESS** = new `balanceStarts()`: measure what each start can REACH (road-legal BFS out
  ...
  of the disc), then top the poor up and trim the rich to a Â±22% band. Wood/stone are counted
  ...
  count of 1; (d) lifting a bare start to a forested neighbour's budget BURIES it â€” one board
  came out with a single legal plot in its whole territory, hence the density caps and the new
  `openCastleApproach()` playability guarantee (road-legal reach â‰¥ REACH_MIN AND â‰¥ PLOTS_MIN
  ...
  most of its free ground) â€” it is deliberately NOT banned.
- **START SITES** now require minable rock within `MOUNTAIN_MAX` by ROAD-LEGAL walking
  (`distFieldRoad`, new) â€” crow-flies distance happily crosses a lake â€” and a wide land share
...
- Green space: `SEA_BOWL` 0.75 â†’ 0.38, `WATER_N` 0.27 â†’ 0.21, `SWAMP_MOIST` 0.62 â†’ 0.72,
  `DESERT_MOIST` 0.16 â†’ 0.11, `FOREST_T` 0.50 â†’ 0.56, `ROCK_T`/`ROCK_P` eased.

**5 Â· SUITABILITY OVERLAY IS A PROPERTY OF THE MODE.** `syncSuitOverlay()` is now the only
...
`filteredSuitFromBuild` latch is gone â€” it was what let the overlay outlive its mode.

**6 Â· PLACE â†’ CONNECT.** A finished building hands you the ROAD tool with its own door flag
...
**7 Â· AI DIFFICULTY** â€” `FSC.AI_DIFF` easy/normal/hard OVER `AI_PERSONA`, exactly the Farm
...
demand before attacking, attack cadence, committed share, and â€” defensively â€” extra occupancy
...
into MP through the host's `settings` block â€” a guest can never run different opponents from
its host. UI: a Rival-skill segment beside Rivals, persisted in `localStorage fs_diff`.
Measured at 12 in-game minutes on medium/12345: easy 8 bld / 550 land / 1 mil / 5 knights Â·
normal 12 / 551 / 4 / 9 Â· hard 17 / 588 / 7 / 15 â€” monotonic on all four.

**8 Â· BUILDINGS ARE SOLID** (the reported "villagers walk through the castle"). Root cause was
NOT `passable()`: `map.bldAt` only ever marked a building's ANCHOR, so five sevenths of a
castle was open meadow to every pathfinder. New `map.bldFoot` marks the BODY â€” the ring of a
...
â€” a start that cannot build. Hence `DOORSTEP_R` (two rings of clear doorstep) and
`openCastleApproach`. Worst-case road-reachable across 72 kingdoms: 13 â†’ 43+.

**9 Â· TREE SWAY** â€” `onBeforeCompile` on the tree materials only: displacement masked by the
...
at the canopy â‰ˆ four degrees on a 2-unit pine. Zero draw calls, zero geometry, zero FSC.rng.
`FSRender.setTreeSway(false)` freezes the clock dead (verified frame-stable) and the FILM-STRIP
RIG disables it â€” those sheets exist to judge one thing and 24 frames of drifting foliage is
noise in every one. Default ON in the game. Measured 10.3% of pixels differ between stills a
second apart.

**10 Â· CARRIED GOODS RIDE PER BODY** â€” `FSModels.carryOf()` beside `hipOf`: minifig 0.86 (his
cap crowns at 0.855, unchanged since Phase P), villager `VILLAGER.TOP_Y + 0.012`.

**â­ USER PREFERENCE, same day: THE ORIGINAL MINIFIG SERFS ARE THE DEFAULT AGAIN.** After
seeing the Tripo villager in game the user asked for the procedural people back. Nothing was
deleted â€” `cast.on` is now `false`, the boot-time `loadCast()` is gated, and a DEFAULT BOOT
...
`?cast=1` (which also writes the flag). The GLBs stay in `assets/farmstead/cast/villager/` â€”
the sprite-impostor exploration uses them too. visuals Â§6a is the new default-path section;
Â§6b runs the whole villager battery behind the opt-in, blocked-GLB fallback included.

**SUITES**: world **81/81** (+6: separation, mine-legality, green share, per-resource fairness,
determinism) Â· transport **131/131** (+7: the solidity section) Â· economy **104/104** Â· visuals
**113/113** (+7 sway, +6 default-cast, +1 villager carry height) Â· visual **56/56** Â·
military **124/124** (+9 difficulty) Â· ui **128/128** (+8 placement flow) Â· **mp 126/126** â€”
...
`pickVertex` sky probe (the island now fills a steep close view â€” it sweeps for real sky),
`T.flagSpot` (shortest road, not first found â€” roads detour around buildings now and a 16-edge
road blew every fixed tick budget), the fish-migration seed (needs â‰¥2 open MIGRATE directions),
the pig-breeding window (a lottery needs more than 14 cycles), `T.spotsNear` (never stage on an
ENEMY TEAM's ground â€” ownership was only lent for the terrain test, then the conquest cascade
burned the fresh building down), the boat crossing (re-plot after wiring the near shore), the
cramped-AI seed 777 â†’ **22** (777 opened up to 69 plots; 22 is 17, against a median of 103),
the AI-war seed 12345 â†’ **101** (12345's starts are no longer close enough to fight), the
cross-siege geometry (equal-distance sources, and read the LOSER off the result â€” which column
wins is the map's choice), and the `legPools` count (a ceiling: a pool only exists once
something has drawn from it). `SIM_BASELINE` in the visual suite was RE-BASELINED â€” golden
...
# ðŸ§ FARMSTEAD FORK B â€” baked cast sprites (2026-08-01, PRODUCTION SHEETS, awaiting in-game integration)
...
pipeline + the shipped sheets. NOTHING is wired into the game yet â€” a later agent does the
...
**THE SHEETS**: `assets/farmstead/cast/sprites/` â€” `serf-body.png` 1536Ã—1920 (582 KB) Â·
`serf-mask.png` 768Ã—960 (47 KB) Â· `knight-body.png` 1536Ã—1920 (660 KB) Â· `knight-mask.png`
1536Ã—1920 (206 KB) Â· `overlays.png` 1536Ã—2944 (259 KB) Â· `manifest.json` (142 KB, minified â€”
1800 anchors) Â· `README.md` (the schema, generated FROM the manifest so it cannot drift).
**1.75 MB of PNG (1.9 MB with the manifest), 53.8 MB as RGBA8 in VRAM, 12 azimuths, ONE
pitch row at 52Â° (`FSC.CAM.PITCH_START`), world-fixed sun.** Levers if that is too much:
...
mask sheets â€” verified end-to-end, then deleted. It skips the overlay half only because that
...
`cameraYaw âˆ’ unitYaw`. One frame no longer has to serve every (facing, camera) pair with the
...
**THE COMBINATORICS SOLUTION** (the actual deliverable â€” per-job sheets would be 23 jobs Ã— 4
players, plus 5 ranks Ã— 4 players):
...
2. **Mask sheet**, same grid, unlit: **R = team, G = knight rank trim**. Formula in the
   ...
   The emissive add-back is why tinted regions are EXACT and not `emissive*(1âˆ’tint)` too dark.
   maskDiv is PER SUBJECT â€” serf 2 (one chunky sash), **knight 1** (a 2-3 px plume/rim against
   the team shield bleeds R into G at half res: a gold rim comes out pink).
3. **Overlays** â€” hat, pack, 10 tools, rank pip â€” own 64 px cells on the same azimuth grid,
   posed on the body. **The cap geometry is IDENTICAL for all 23 jobs; only `FSC.JOB_COLOR`
   differs â€” so ONE white-baked hat sheet tinted per job collapses 23 sheets into 1.**
4. **Anchors are GENERATED**: every body cell projects its 3D mount points (hat/tool/pack/carry,
   ...
   is `overlayTopLeft = bodyAnchor âˆ’ overlayPivot`. 1800 anchors, none hand-authored.
...
- **Turn-pop** during a real hex heading change at `SERF_TURN_RATE` 7.5 rad/s (7.16Â°/frame),
  REAL sheets baked at each count. Peak jump is flat across counts (small-sample max); the
  statistic that matters is **pop/mean, the stepping signature**: 6 â†’ **9.8**, 8 â†’ **7.1**,
  12 â†’ **5.0**, 16 â†’ **3.3** (3D mesh reference: 1.3). At the default zoom the unit is 12 px
  and 12-azimuth pop is 0.033/255 over the crop â€” invisible.
- **Static hex-lattice error** â€” the decisive one. A serf rests on one of six 60Â° headings, so
  an azimuth count that does not divide 60Â° renders a STANDING serf permanently wrong.
  6/12/24 are exact; **8 is off by 15Â° forever (image diff 0.217 at max zoom), 16 by 7.5Â°
  (0.125)** â€” 8 and 16 are disqualified outright. Among the exact counts, 6 has twice 12's
...
**POSES ARE THE GAME'S OWN** (frozen `fs-render.js`, nothing invented): serf idle Ã—1 Â· walk Ã—8
(`|cos(phase)|*0.052` bob, 0.06 lean, Â±0.10 twist, âˆ“0.055 roll, legs Â±0.52 rad about the hip) Â·
work Ã—6 **uniform in `serfSwing`'s 0..1** so `k = round(swing*5)`. Knight guard Ã—1 Â· walk Ã—8 Â·
fight Ã—6 keyed by `duelPose`'s **`l`** âˆˆ [âˆ’0.34, 1.0] (frames carry their `l`; pick nearest).
**Deliberately NOT baked, and why**: carry-walk (the frozen `drawSerf` does not pose a carrier
differently â€” only a baked pack + a separate crate mesh, so those cells would be duplicates) and
a knight fall (**the game has none** â€” a dead knight becomes `corpseGeo`, a flat mesh that
scales away). Each fight frame also carries `lungeOffset`, the WORLD displacement toward the foe
that `knightVisual` applies â€” the sprite does not contain it, the integration must.

**BUGS THIS COST, all worth knowing**:
- **Bounds by world-AABB corners are wrong for a subject that rotates.** The yawed knight's
  world AABB is a square of side 2Â·maxRadius whose corners are empty air; projecting them
  inflated the shared frustum ~70% and left the serf filling 40% of his cell. Project real
  VERTICES â€” halfSpan 1.108 â†’ 0.650, resolution +70%.
...
- **Overlays must be baked behind a DEPTH-ONLY copy of the host body**, or a tool on the far
  ...
  front, a chest pip from behind) â€” they carry `"empty": true`; skip the draw.
- **Every per-instance attribute needs `needsUpdate` every frame.** One missed (`aTint2`) kept
  whatever it held at first upload while instance indices changed meaning underneath â€” it read
...
94.09), for the whole bake), not the lowest opaque pixel â€” the minifig has depth and a pitched
...
**PREVIEW / ACCEPTANCE**: `farmstead-proto/sprite/forkb.html` â€” yaw-locked camera over the real
...
draws the FROZEN merged 3D builder beside the sprite â€” that is how the bake was judged, not by
eye. **VERIFY: `node farmstead-proto/sprite/_verify_forkb.cjs [--shots] [--measure]` â€” 143/143,
...
load-bearing â€” the game's camera yaw must equal `manifest.bake.cameraYaw` (0) or every sprite
...
layer); lift it rather than re-deriving the anchor arithmetic. Contact shadows are NOT baked â€”
...
## ðŸ§ FORK B IS IN THE GAME â€” sprites by default, camera locked (2026-08-01)
...
kept underneath as an automatic fallback. The LOOK is unchanged â€” the sheets were baked from
the minifig â€” so `shots/fs_forkb_sidebyside.png` (3D left, sprites right, one camera, one
...
Sheets fail to arrive â†’ `SPR.ready` stays false and the 3D path carries the settlement,
...
`localStorage fs_sprites="0"`, `FSRender.setSprites(false)`. `?sprites=1` opts back in â€” and,
...
and it is an ASSIGNMENT, not a clamp â€” `setCam({yaw})` from a save, a gesture or a
...
vertical ORBIT look; it now flips the vertical PAN â€” same preference, applied to the one axis
that still moves.
**PITCH is a MEASURED band, 49Â°..58Â°** around the bake's 52Â°. Measured in-game, not guessed
(scratchpad `fs_pitchband.cjs`): one still serf, same world and camera rendered twice â€” sprites,
then the 3D fallback the sheets were baked from â€” differenced over a 110 px crop, swept 30Â°â†’74Â°.
At the closest legal zoom the mean |Î”RGB| is a clean V with its minimum EXACTLY at the bake
pitch: 30Â° 5.73 Â· 40Â° 4.89 Â· 46Â° 3.60 Â· 50Â° 2.17 Â· **52Â° 1.21** Â· 56Â° 2.20 Â· 58Â° 2.40 Â· 64Â° 2.74
Â· 74Â° 3.18. 1.21 is the bake's own floor (cell resolution, AA, and the constant depth bias); the
...
whole sweep is under 0.4 â€” invisible either way. Pitch has no player input any more; the band
exists so a saved or staged camera can be nudged for framing.

**THE RENDERER** (fs-render.js, `SPR` block â‰ˆ 300 lines): three InstancedMeshes â€” serf bodies,
knight bodies, overlays â€” so the WHOLE workforce is **2-3 draw calls**. `forkb-preview.js`'s
...
â€” the walk's Â±0.10 twist is already IN the cell; walk k from `vis.phase`; work k from
...
carried crate (seated on the sheets' own `carry` anchor â€” `(footPx.y âˆ’ anchor.y)/(ppuÂ·cos pitch)`
...
The shader multiplies `(âˆ’anchor + off + quad)` by `aScale` as one term, so pre-scaling the
overlay offset shrank every hat toward the feet by `scaleÂ²` while the body shrank by `scale` â€”
...
|---|---|---|---|---|---|
| sprites | 81 | 272,235 | **470** | 2 layers | 0.8â€“1.7 ms |
| 3D fallback | 93 | 305,777 | **33,892** | 13 pools | 1.6 ms |

72Ã— fewer triangles for the cast. NOTE the first frame after a layer rebuild is a ~100-170 ms
SwiftShader SHADER COMPILE â€” warm up past it or a perf table measures the compiler (steady
state re-measured over 3Ã—60 frames: worst 1.7-2.2 ms).

**SUITE RESHAPE.** visuals 113 â†’ **142**: Â§6a is now "the default cast is the sheets" (proven on
the WIRE â€” all six files fetched, villager GLBs still never asked for â€” plus zero 3D people
pools and â‰¤3 draw calls); NEW Â§6b is the sprite contract cell by cell (12 azimuths at the baked
pitch Â· all six hex headings land on an EXACT cell with 0Â° error and the azimuth wraps Â· the
walk frame advances one step at a time through the whole cycle Â· work frames uniform in the
swing Â· nearest fight frame incl. guard Â· all 1800 generated anchors inside their cells, all 973
pivots too, every composed placement on the body Â· all 119 hidden cells skipped by the DRAW
PATH's own resolver) plus a repaint probe that reads the two tint channels off the SCREEN (team
colour swapped redâ†”green: pixels move, only 1.7% of the man's, and they take the colour they
were given) and a door-fade probe that reads the `aScale` attribute the shader actually
consumes; Â§6c/Â§6d/Â§6e keep the 3D path honest â€” sprites-off hands the workforce back to the
...
locked, 0 page errors). visual 56 â†’ **60**: the leg-pool budget check became sprite-aware, and a
new Â§2b covers the frame-step signature. Camera-lock ripples were restaged, not bent: world's
...
yaw" became "can no longer turn the world" + "â€¦and still pans"; transport's "one mesh per
job+player" and military's "two knight pools" now assert whichever path is live.
**FILM STRIPS render in the SHIPPING default (sprites)** â€” the strips exist to judge what the
...
play constraint); pitch is NOT overridden any more â€” it is a rendering contract, and a strip
shot outside the band would judge a pose the game cannot show. All 10 reviewed strips
regenerated (gait CV 0.107, jump 1.02Ã—, yaw 0 â€” unchanged).

**FULL BATTERY, all green**: world 83 Â· transport 131 Â· economy 104 Â· visuals 142 Â· visual 60 Â·
military 124 Â· mp 126 Â· ui 128 Â· polish 68 = **966 checks, 0 page errors**. (polish's known
...
loader â€” no retry); the depth bias makes a sprite ~3% larger than the mesh at `DIST_MIN`
...
## ðŸ§ª TEST/exploration â€” two downloaded GLBs through the Fork B pipeline (2026-08-01, look test only, NOT shipped)
...
`assets/farmstead/cast/sprites-test/` (baked output: `villager-body.png` 1536Ã—1664,
`knight-body.png` 1536Ã—1280, `manifest-test.json`, `README-test.md`, `parts/` = the
normalized/split GLBs + per-model `*-measurements.json`, `_inspect/` = Blender preview
renders + logs from the exploration itself) Â· `farmstead-proto/sprite/fs-cast-bake-test.js`
+ `bake-test.html` (sibling bake harness, same 12-az/52Â°/world-fixed-sun/one-locked-scale
recipe as `fs-cast-bake.js`, never imports it) Â· `tools/_fs_bake_sprites_test.cjs` (driver)
Â· `tools/_fs_spritetest_{inspect,splitpreview,splitparts,assemblecheck}.{py,mjs}` (Blender +
Node normalize/split pipeline) Â· `farmstead-proto/sprite/forkb-test.html` +
`forkb-preview-test.js` (copy of forkb.html's preview, own sibling JS, SOURCE toggle
minifigâ†”test) Â· `farmstead-proto/sprite/_verify_forkb_test.cjs` â€” **84/84, 0 pageerrors**,
...
+Z (measured off raw vertex data â€” head-band |Z| extremes are further +Z than âˆ’Z on both,
i.e. nose-forward â€” no yaw correction needed, confirmed independently of trusting Blender's
axis-conversion table by eye). **Dwarf: SKINNED**, the exact same Tripo v1.0-20240301 41-joint
biped rig as `assets/farmstead/cast/villager/` (same joint names, `L_Thigh`/`L_Calf`/etc.) â€”
7399v/8529t, one stray 42-vert Icosphere object also in the file (marketplace leftover,
excluded by picking the largest-vertex-count primitive). **Knight: NO skin at all** â€”
8064v/8830t, single static mesh.

**LEG SPLIT â€” one clean, one honest fallback** (both attempted, judged from real Blender
...
- **villager = TRUE SPLIT** (`posingMethod:"split"`). Same recipe as the shipped villager:
  ...
  ToeBase/Â±Twist chains), each leg recentred on a measured hip pivot (generalized as "centroid
  of the closest-to-parent 12% band" â€” works without a skeleton too, see below), rigid GLB
  ...
  minifig's tunic is a deliberately bell-flared sphere-hem FOR this technique â€” REPORT.md
  says so; a realistic tailored tunic has no such slack) â€” visible in a magnified Blender
  render, much less so baked down to a 128px cell at 52Â° pitch (see the walk crops).
- **knight = WHOLE-BODY FALLBACK** (`posingMethod:"wholeBodyBob"`). A geometric hip-line
  ...
  the greaves â€” armor has no cloth "give" to hide a rigid-part seam the way the villager's
  tunic does. Shipped instead: guard(1, whole body) + walk(3, vertical bob + gentle lean
  only, no leg articulation â€” deliberately subtle, a big bob reads as floating not walking)
  + **fight(6, the REAL unmodified production `knightPose('fight')` torso-lunge formula** off
  `duelPose`'s `l` â€” that pose is pure bob/pitch/roll, needs no legs at all, so "knights
...
**ARM RE-POSE** (found, not asked for â€” the task brief only covered legs): baking the raw
T-pose gave a spread-eagle sprite because the ~1.0-unit ARM SPAN, not the ~0.79-0.99-unit
standing HEIGHT, drove the locked-scale frustum â€” a starfish that fills maybe 15% of its
cell. Both arms rotated DOWN âˆ“75Â° about a measured shoulder pivot (same "closest-band
centroid" technique as the hip pivot â€” skin-weight Clavicle+Upperarm+Forearm+Hand chain for
...
normalize pipeline in Node via `@gltf-transform/core` â€” NOT Blender â€” deliberately: staying
...
â€” always pass absolute paths to `bpy.ops.render.render`'s `filepath`.
...
three â€” a valid, common glTF layout, and THREE's GLTFLoader correctly parses it into a
`THREE.InterleavedBufferAttribute`. But that attribute's raw `.array` is the WHOLE
interleaved buffer (8 floats/vertex here), not a clean `[x,y,z,x,y,z,...]` run â€” the bake's
...
Symptom: some "positions" landed near Â±1.0 (actually normal components), which blew the
locked-scale frustum out ~3.4Ã— (pxPerCameraUnit 33.66 vs a correct 114.57) â€” sprites baked
at roughly a third of their proper size, most of every cell wasted as padding. Silent and
very hard to spot from the manifest alone (nothing errors; the numbers are just wrong) â€” only
...
subtype â€” correct for both interleaved and plain geometry, no need to know which a given
...
between looks in place" SOURCE toggle originally called `spawn()` on every switch â€” meaning
...
and the interleaved-buffer bug were both fixed (`shots/fs_spritetest_sidebyside.png` â€” same 4
...
flatter toon colours (expected â€” these are textured, not vertex-painted-toon, source assets).
...
the dwarf's v1.0 skeleton) â€” same ownership standing as the API-generated cast, commit-safe
when adopted; (2) no team-tint masks and no hat/tool/pack overlay anchors exist for the
villager (`manifest.overlays = {}`, every subject's `mask = null`, by design â€” out of scope
...
**Verify**: `node farmstead-proto/sprite/_verify_forkb_test.cjs [--shots]` â€” 84/84, 0
...
## ðŸ§” THE DWARF+KNIGHT LOOK IS THE DEFAULT â€” sprites, skinned, minifig one flag away (2026-08-02)
...
rebuilds the layers in place, awaitable). Both URL forms WRITE localStorage, so a link sticks â€”
...
**THE KNIGHT IS SKINNED NOW â€” that is the whole upgrade.** The look test split him into rigid
...
`tools/_fs_dk_rig.py` gives him a real skin by **skeleton transfer** â€” the dwarf's own Tripo
...
- **The fit is measured, not guessed** (`tools/_fs_dk_measure.mjs` â†’ `landmarks.json`): dwarf
  H 0.9049 / crotch 0.2149 / arm line 0.5618 / leg centres Â±0.105 Â· knight H 0.9935 / crotch
  0.2277 / arm line 0.5837 / leg centres Â±0.106. Horizontally they are the SAME character
...
- **Bone heat failed at first and it was a MESH problem, not a rig problem.** `parent_set` reports
  the failure as a WARNING, returns FINISHED and leaves an UNWEIGHTED mesh â€” which exports with
  no skin and reads downstream as "the knight just does not animate". Root cause: the Tripo
  export splits every vertex (8064 verts for 8830 tris). `remove_doubles` â†’ 4403 verts â†’ heat
...
- **Plates must not rubber-band**: `*Twist* + Root` marked non-deform before binding, weights
  limited to 2 influences, and vertices a Clavicle/Head already OWNS locked to it outright â€”
  without that last step, dropping the arms 72Â° out of T-pose stretched each pauldron into a
...
- The knight looked blown out to me, so I nearly dropped the emissive lift. **The numbers said
  do not**: minifig knight 185.2 mean / 32.3% blown vs dwarfknight 169.2 / 15.4% â€” the SHIPPED
...
- **Per-body stride**: knight `strideMul` 0.62 â€” a rigid fauld at the full 0.52 rad reads as a
...
- **Arm counter-swing** (0.30 rad) is the ONE addition beyond the game's own pose maths, and it
  rides on `p.stride` â€” the game's own `sin(phase)` â€” so it adds no clock and no state. A sculpt
...
measured torso span (crotch â†’ arm line), restricted to torso-dominant BONES so the T-pose arms
...
armour â€” a geometric "above and behind the helm" rule would also catch the helmet's back).
Regions bake WHITE with no emissive lift, exactly like production, via a per-triangle vote that
reorders the index into three geometry GROUPS (plain/team/rank) sharing one skin â€” the textured
...
- **Anchors come off the posed skeleton where the pose moves them**: `tool` follows the HAND
  ...
  `headTop âˆ’ 0.25Â·headRadius` â€” feed it the minifig's own numbers and it returns 0.766, its
...
- **Overlays were REBAKED, not reused** â€” deliberately. The existing sheet's occlusion is cut
  ...
  unchanged; only scale (measured: hat Ã—1.05, pack Ã—0.97) and the occluding host differ.
...
the renderer needs no per-look arithmetic â€” a look really is just a base path. Fitting became an
...
at HEAD height (the arm was rotating UP â€” a +x arm needs a NEGATIVE rotation about +Z to come
...
**SUITES ARE MANIFEST-DRIVEN NOW, not bake-specific.** visuals Â§6a asks the renderer which look
...
(measured 0.239, screen moved 0.264) instead of a typed band. NEW Â§6a-ii is a minifig smoke pass
(sheets load, cells resolve, workforce is sprites) plus a look-fallback check. The sheets-blocked
3D-fallback pass is intact â€” its blocker regex now matches EVERY look, without which it was
silently testing nothing.

**FULL BATTERY, all green, first pass, no flakes** (the known polish/mp flakes both passed):
world 83 Â· transport 131 Â· economy 104 Â· **visuals 148** (142 â†’ +6) Â· visual 60 Â· military 124 Â·
mp 126 Â· ui 128 Â· polish 68 = **972 checks, 0 page errors**.

**Files**: `tools/_fs_dk_{measure.mjs,rig.py,review.cjs,shots.cjs}` Â· `assets/farmstead/cast/
dwarfknight/` (src GLBs â€” the user's own Tripo generations, commit-safe â€” plus `{dwarf,knight}-
rigged.glb`, `landmarks.json`, `rig.json`, `_inspect/`) Â· `assets/farmstead/cast/
sprites-dwarfknight/` Â· `fs-cast-bake.js` gained a `dwarfknight` source through its existing
...
`serf-body 5ea45cb6â€¦`, `serf-mask 0bee11b9â€¦`, `knight-body 6cc7d18aâ€¦`, `knight-mask f408d8feâ€¦`,
`overlays 0134ec85â€¦`, `manifest 64aa3a04â€¦`, `README 47bb4c6dâ€¦`.
...
the shipping default, so `node tools/_fs_filmstrip.cjs <subjectâ€¦>` should be re-run to refresh
them â€” the visual suite only asserts they exist and are not blank, so it stays green either
...
band â€” a luma-modulated dye would keep them but breaks the manifest's "tinted pixels match the
3D game EXACTLY" contract.

## ðŸŽ¯ FARMSTEAD PLAYTEST BATCH #2 â€” five items from one session at the wheel (2026-08-01)

**1 Â· TEAM COLOUR IS THE BELT, AND A KNIGHT'S IS HIS PLUME** (both looks, re-baked).
...
- **The dwarf's belt was MEASURED, not guessed.** Binning his torso texture colour against
  ...
  f 0.16..0.32 (mean rgb 91,68,48 / 82,61,42, Râˆ’B â‰ˆ 40) against a neutral grey tunic
  everywhere else (105,99,92, Râˆ’B â‰ˆ 13). The old `[0.20, 0.40]` "waist sash" band sat ABOVE
  the belt he actually wears.
- **Both bands ship WIDER than the thing they name, and here is the arithmetic.** A serf is
  ~26 px at play zoom, so a band that is a fraction *f* of his height is 26Â·*f* px. The
  minifig's authored belt is 0.045 of a 0.79-unit body = 5.7% = **1.5 px** of team colour;
  the dwarf's drawn leather is 6.1% = 1.6 px. Neither reads. Shipped: minifig belt 0.045 â†’
  **0.095** tall (12% â‰ˆ 3.1 px; centre 0.262 â†’ 0.30 so the whole band sits ON the tunic
  instead of hanging off its hem) and the dwarf's band **[0.10, 0.40]** of the torso span
  (11.5% â‰ˆ 3.0 px, centred 0.25 on his belt's 0.24). That is the read the old diagonal sash
  had â€” its ROLL swept ~5 px of vertical extent, which is why a flat band of the same
...
- The minifig's chest baldric is GONE; its surcoat, shield face and back disc lost the team
  tint and took their own colours â€” the surcoat had to be toned from linen 0xd6ccb6 to
  0xb9b09b, because at linen white it out-shone the armour and read as a blank card where
  the colour used to be. Its plume boxes widened 0.05 â†’ 0.075 / 0.04 â†’ 0.06 in x for the
  same 1.6 â†’ 2.5 px reason.
- The knight's rank trim became a waist band, **narrowed [0.24, 0.50] â†’ [0.30, 0.44] after
...
- The SHOULDER YOKE went with the sash. It was there for the overhead camera and a belt is
  more self-occluded from above â€” that is the cost of the new language, paid deliberately.
- KNOWN: a carrier's PACK covers part of the belt from behind (the shot stages builders for
  ...
  a handful of reddish boot vertices â€” a couple of pixels, present before this change too,
  when they were rank-coloured instead of team-coloured.

**2 Â· A BUILD CARD IS GREY IF AND ONLY IF YOU CANNOT AFFORD IT.** The panel used to re-grey
...
while you were reaching for it â€” the reported "weird highlighting mechanics".
...
the refresh rides the goods ticker's â‰¤4 Hz beat: a delivered plank un-greys a card with no
hover, click or panel re-open. Suitability did not disappear â€” it lives where it can actually
...
**3 Â· CONSTRUCTION FELLS THE WOOD** â€” and the research says the original was HALF of what the
user remembered. Full write-up in `farmstead-plan.md` Â§14.13; the short of it, from freeserf's
...
- **Roads ran straight THROUGH standing trees in the original.** `is_road_segment_valid`
  ...
  removed the tree â€” the road was laid under it. Farmstead was **stricter than the original**
...
a woodcutter's stump). NOT cleared: stone piles and standing crops â€” a pile is the
...
is the legality half. The felling animation was FREE â€” `dirty(G,v)` already makes
...
plots within radius 14), seeds 12345Â·7Â·42Â·1234: reach **311â†’375, 324â†’408, 350â†’378, 354â†’373**;
plots **129â†’183, 94â†’141, 151â†’164, 227â†’243**.
...
**4 Â· THE STONECUTTER WALKS TO THE NEAREST ROCK** (plan Â§14.14) â€” alone among the professions.
...
LOWEST VERTEX INDEX â€” `forRadius` walks ROW-MAJOR, not ring-major, so the winner has to be
...
**5 Â· THE CAMERA TURNS AGAIN; THE PITCH STAYS BANDED.** Fork B's yaw LOCK is gone â€” `clampCam`
...
baseline precisely so restoring it would be two lines â€” it was). Pitch inputs are wired but
`clampCam` holds them inside the MEASURED 49Â°..58Â° band, unchanged. "Invert vertical drag"
...
- **THE SHEETS HAD TO BE RE-BAKED, and the reason is physical.** A locked camera lets a cell
  ...
  index RELATIVE â€” `sprAz` subtracts the live camera yaw â€” and one cell then has to serve
...
- **HOW WRONG IS CAMERA-RELATIVE LIGHT? Measured, and the answer is: barely.** The bake
  ...
  camera-space direction â€” the shipped key is exactly that, which means the cast lights
  ...
  were baked from, 110 px crop at dist 10, mean |Î”RGB|): **0Â° 1.93 Â· 45Â° 2.14 Â· 90Â° 2.69 Â·
  135Â° 3.01 Â· 180Â° 2.81 Â· 225Â° 2.44 Â· 270Â° 2.32 Â· 315Â° 2.33**. The floor is the bake's own
  ...
  zoom. **The user's authorised fallback levers â€” soften the global rig, or drop the
  directional sun and the blob shadows entirely â€” went UNUSED.** There was no mismatch to fix,
...
- **AZIMUTHS 12 â†’ 16, with the measurement.** 12 was chosen because a serf rests on one of six
  60Â° hex headings and 12 divides 60Â° exactly, so a STANDING serf was never rendered at the
  wrong angle â€” an argument that only holds under a LOCKED camera. Free rotation makes the
  ...
  a still unit, camera orbiting it in 2Â° steps over 120Â°, frame-to-frame mean |Î”RGB|):
  **16 â†’ mean 2.55 / worst step 8.95 / pop-over-mean 3.51 Â· 12 â†’ 2.47 / 9.42 / 3.82 Â· the 3D
  mesh â†’ 3.08 / 3.44 / 1.12.** 16 is smoother on both statistics, and the in-game 3.51 lines
  up with the demo's controlled `measureTurnPop` figure for 16 (3.3, against 5.0 for 12).
  Sheets grew as budgeted: minifig 1.75 â†’ **2.21 MB** (+26%), dwarfknight 2.70 â†’ **3.37 MB**
  (+25%); 1812 â†’ 2416 cells, 1800 â†’ 2400 anchors.
...
**SUITES.** world **83** Â· transport **138** (+7, the tree-clearing section) Â· economy **107**
(+3, the stonecutter's near-then-far ordering run twice for determinism) Â· visuals **151**
(+3: relative-azimuth sweep, one-camera-step-is-one-cell-step, overlays-follow-the-camera) Â·
visual **60** Â· military **124** Â· mp **126** Â· ui **128** Â· polish **68**. Proto suites kept
green: `_verify_forkb` **142/142** (its hex-lattice-exactness check restaged â€” that rule only
...
workshop quarter went `sep` 3 â†’ 4 â€” its site picker is greedy-NEAREST, and with forest ground
...
connect; military's cramped-AI seed was re-scanned for the THIRD time (seed 22 went 17 â†’ 84
...
two hut-build flows staged a yaw of 0.6 that the LOCK had been quietly overruling to 0 â€” with
...
sim changes moved them â€” the stonecutter stopped drawing a random disc sample, so the whole
...
would still count as MOVING after the clock stops â€” he usually does not (his `frac` never
...
unchanging cell, which is strictly stronger about the property under test â€” a cell that
cycled would fail it â€” and indifferent to how long the settle takes.
**RE-BASELINED sha256** for `assets/farmstead/cast/sprites/` â€” a deliberate rebake, not drift:
`serf-body 105ce4b6â€¦`, `serf-mask f0c46f26â€¦`, `knight-body 592c48ecâ€¦`, `knight-mask
7bb93b79â€¦`, `overlays c8b9f2deâ€¦`, `manifest d7b003cdâ€¦`, `README e84ea851â€¦`.
...
`node tools/_fs_b2_shots.cjs` â€” 6/6, and it ASSERTS what each shot is supposed to show
...
## ðŸŒ¾ FARMSTEAD PLAYTEST BATCH #3 â€” twelve items from one session at the wheel (2026-08-02)

Every number below is MEASURED, before and after, with the "before" reproduced by running the
same metric through pristine copies of the files (scratchpad `mapstat3.cjs`, 8 seeds Ã— 3 sizes
Ã— 4 starts; `fs_b3_perf*.cjs` for the frame board).

**A1-A3 Â· WORLD GENERATION.** Three changes, one pass of the generator.
- **MAPS ARE TWICE THE AREA** â€” `MAP_SIZES` 48/64/96 â†’ **68/90/136** (side Ã— ~âˆš2, snapped even
  so the odd-row lattice keeps equal numbers of shifted and unshifted rows; area ratios 2.01 /
  1.98 / 2.01). Everything sized in ROAD STEPS is deliberately untouched â€” a serf does not walk
  faster on a bigger map â€” and everything expressed as a fraction of the map moves by itself.
  Castle separation went 12.9/21.8/33.3 steps â†’ **23.4/32.1/51.6** (26.8/34.0/34.6% of the
  width â†’ **34.4/35.7/38.0%**), buildable plots within r14 worst 20/53/68 â†’ **66/92/65**, mean
  113/162/253 â†’ **200/255/303**.
- **LESS WATER, MORE LAND AND MOUNTAIN** (`SEA_BOWL` 0.38 â†’ 0.20, `WATER_N` 0.21 â†’ 0.145,
  `RAD_0` 0.94 â†’ 1.00, `MOUNT_N` 0.745 â†’ 0.715, `SNOW_Y` 5.6 â†’ 6.0). Water 46.3/41.5/37.0% â†’
  **33.2/29.5/25.6**, grass 30.8/33.9/37.3 â†’ **37.8/40.0/42.7**, mountain 12.8/13.4/13.6 â†’
  **17.4/17.8/18.3**, snow 2.4/3.0/3.5 â†’ **2.7/3.1/3.4** (SNOW_Y rose on purpose â€” dropping
...
- **EVERY START IS GUARANTEED THE ORE ITS ECONOMY NEEDS** (`FSMap.guaranteeStartOre`, run inside
  ...
  opens with; the economy does not care how much, it cares WHICH â€” coal and iron together are
  ...
  design at MINERAL_W 2), stone is deliberately NOT guaranteed â€” the stonecutter works surface
  piles and those are already balanced. **"Close" is now ONE number**: `MOUNTAIN_MAX` 25 â†’ **18**
  road steps, shared by the site scorer's soft bar and the guarantee. Measured: starts with coal
  or iron ABSENT 1/0/0 per 32 â†’ **0/0/0**; gold absent 8/0/0 â†’ **0/0/0**; starts without BOTH
  coal and iron inside 18 road steps 9/5/4 per 32 â†’ **1/0/2**; worst road distance to iron
  29/26/51 â†’ **20/18/21**, to gold 40/37/60 â†’ **20/28/24**.
  - **The cheap fix is preferred and is what normally happens: RE-KIND a seam that is already
    ...
    skips them.
  - **The ladder is by DISTANCE, not by strategy** â€” both moves (re-kind, then seed) are tried
    ...
    pass ever ran.
- Fairness improved as a side effect of the bigger boards: spread (maxâˆ’min)/mean wood
  0.71/0.21/0.20 â†’ **0.24/0.17/0.20**, stone 0.75/0.31/0.17 â†’ **0.17/0.15/0.16**, ore
  0.67/0.65/0.59 â†’ **0.32/0.39/0.41**.
- **DETERMINISM RE-PROVEN**: same seed twice = same map hash at every size (3 sizes Ã— 3 seeds),
  different seed always differs, and the world suite's own determinism section is green.
- **PERF AT THE NEW LARGE** (SwiftShader, 1280Ã—800, warmed past the ~100-170 ms shader compile,
  ...
  **336 ms** (terrain mesh 32.5 ms), 9000 sim ticks **285 ms**, frame at play zoom **91.6 ms**
  ...
  terrain mesh itself only went 18,050 â†’ 36,450; the other ~340k is TREE instances, and every
  ...
  and SwiftShader is a CPU rasteriser â€” 1-2 orders pessimistic. **2Ã— side / 4Ã— area (W=192) was
  ...
  triangles become the problem. `CAM.DIST_MAX` was deliberately NOT raised â€” the minimap is the
  overview tool, and zooming further out at a 52Â° pitch mostly buys fog.
- MP: size travels in the host's `settings` block exactly as before; the mp suite is 126/126.

**B1 Â· THE STANDING BANNER IS GONE, and it was keyboard auto-repeat.** "aim at an enemy hut,
tower or castle" existed once, as an error toast in `doAttackAtHover`. Holding **A** made the OS
repeat the keydown ~30Ã—/s, each press pushed a fresh copy with a 4.5 s life, and the strip
renders the last two â€” so two identical panels sat at the top of the screen for as long as the
...
ONE-SHOT â€” placing a flag, swinging an attack, stepping the clock â€” and holding a key is never a
...
**B2 Â· THE "CONNECT TO YOUR ROAD NETWORK?" CHIP IS GONE**, offer and all (`connectOffer`,
...
**B3 Â· EVERY BUILDING SAYS WHAT IT DOES.** `FSC.BLD_DESC` â€” one plain sentence per building,
...
detail strip under the grid, because this is played on an iPad and a finger has no hover state â€”
...
- **THE STRIP'S HEIGHT IS FIXED, and that is load-bearing.** The panel is anchored to the BOTTOM
  of the screen, so anything that changes its height moves every build card upward â€” and because
...
- **The phone panel pays for the strip out of its TAB ROW**: five wrapping 40 px chips cost
  ...
  390Ã—844 screen: 465 â†’ 437 px, still clear of mid-screen, with the grid unchanged at 2 rows.
  Asserted in the suite both ways.

**B4 Â· THE RATE STRIP.** The ticker says how much you HAVE; a hundred planks falling by six a
...
real chain: **plank Â· stone Â· food (fish+bread+meat as ONE number) Â· coal Â· ironOre Â· goldBar**
(the ore, not the bar, for iron â€” what stalls a steel chain is almost always the ore; the bar,
not the ore, for gold â€” goldOre is a step nobody watches). Net figure by default, green up / red
down / grey flat; TAP to split it into what was made and what was used.
- **PER IN-GAME MINUTE off a TICK-based window**, so 4Ã— speed reads exactly the same as 1Ã— â€”
...
- **SIM SIDE**: `G.cons`, the mirror of `G.prod` â€” `{res: total ever consumed}`, written by ONE
...
- **What sits under the top bar is positioned from the bar's MEASURED height** (`--fs-topbar-h`,
  set on the â‰¤4 Hz ticker beat) rather than from a constant tuned for one layout â€” the ticker
...
**C1 Â· THE FISH LANDS WITHOUT A BLACK CIRCLE â€” and the ring was fading the wrong way.**
...
water. The landing ring was the widest particle in the game (0.5 â†’ 1.9 world units over 0.62 s),
...
colour ramp mean what it was always written to mean â€” toward black = toward invisible â€” so the
take-off ripple and the warehouse delivery glimmer cannot go dark either.

**C2 Â· THE WORLD RUNS ON GAME TIME.** Tree sway, grass tufts, the water sheet, the caustic
shimmer, the shoreline surf and the whole of FSFX's ambient life (fish, birds, leaves, dust) now
advance with the SIM: 4Ã— speed is 4Ã— the sway and 4Ã— the waves, and a PAUSED world is a still
photograph. `tAccum` (wall time) survives for exactly one thing â€” the selection ring's pulse,
because a UI affordance has to keep breathing while the game is paused or it reads as broken.
- `ambT` is read off the sim, `(G.tick + tickU) Â· TICK_S`, not accumulated, so it cannot drift
...
- FSFX is handed `dt Ã— speed`, and its `dt = Math.min(0.1, dt || 0.016)` was turning a genuine
  ZERO into a sixtieth of a second â€” a paused world kept boiling with jumping fish.
...
**C3 Â· THE WATER SPARKLE IS GONE.** 240 winking sun glints really are the one thing on a calm
...
uses them, and re-adding the layer is one block). The water still moves â€” base sheet, caustic
shimmer and surf, all on game time.

**D1 Â· THE GOODS ARE THEIR OWN LITTLE SCULPTS, BAKED.** All 26 used to draw as the SAME 0.30-unit
...
(`FSModels.goodGeo`, 26 procedural chunky-toon builders in the house language â€” primitives,
...
into `assets/farmstead/goods/` â€” **goods.png 512Ã—1664, 122 KB**, 26 rows Ã— 8 azimuths of 64 px
cells, plus a manifest and a generated README.
- Same rules as the cast sheets, so a good sits with the people: 52Â° pitch, CAMERA-RELATIVE key
  light (the camera turns, so one cell serves every (object yaw, camera yaw) pair sharing that
  difference â€” `loadGoodSprites` REFUSES a world-fixed manifest), `flipY = false`, view-aligned
  quad, one world-distance depth bias, dPR-1 bake, `readRenderTargetPixels` to drain.
  **8 azimuths, not 16** â€” a good is near-rotationally-simple and 24 px on screen.
...
- **THE FIRST BAKE REPORTED EVERY GOOD FILLING EXACTLY 0.91 OF ITS CELL**, which is what a bug
  ...
  fixed the fill numbers are the review tool, and they drove a second pass on the weak models â€”
  ...
  a raised prow. Fill spread 0.38-0.91 â†’ **0.51-0.91**. Judged from `shots/fs_b3_goods_atlas.png`
  (a labelled contact print, `--contact`) and from a flag piled with six goods at play zoom.
- Per-good source: **all 26 procedural**, in `fs-models.js` â€” plank, stone, lumber, boat, sword,
...
**D2 Â· A SETTLER STANDING STILL IS NOT A STATUE.** The idle pose was ONE frame, so a busy
junction was a shop window of mannequins. The sheets now carry **three slow idle loops of three
frames** (weight shift Â· look around Â· small stretch), expressed entirely in the pose scalars
...
- Deliberately bolder than realistic: a serf is ~26 px at the default zoom, so a 1Â° lean is a
  third of a pixel. These are 3-5Â°, which moves a head about a pixel at play zoom.
- **Which loop a serf stands in is decided by his own id through the RENDER layer's hash â€” never
  ...
  is a still photograph and 4Ã— fidgets four times as fast.
...
- **SHEETS RE-BAKED, both looks** (`node tools/_fs_bake_sprites.cjs` then `--source dwarfknight
  --out â€¦`): idle rows 1 â†’ 9, cells 1812 â†’ **2672**, anchors 1800 â†’ **2912**. minifig 2.40 â†’
  **2.83 MB** (+18%, all of it `serf-body` 733 â†’ 1126 KB and `serf-mask` 65 â†’ 99 KB); dwarfknight
  3.56 â†’ **4.23 MB** (+19%, `serf-body` 1216 â†’ 1849 KB). Knight and overlay sheets are unchanged
  byte-for-byte â€” the knight has no idle block.

**SUITES** â€” all nine green, **997 checks, 0 page errors**: world **84** (+1: the separation
check split into an absolute-steps bar and a floor bar) Â· transport **138** Â· economy **107** Â·
visuals **155** (+4: water freezes when paused, the glints are gone, the sway is on game time
and freezes) Â· visual **60** Â· military **124** Â· ui **135** (+7: the info strip's copy, its
no-clip contract at both viewports, "filling the strip never moves a build card", and the connect
chip's removal) Â· polish **68** Â· mp **126**. Plus `node tools/_fs_b3_shots.cjs` **27/27** (it
asserts what each of the eleven shots is supposed to show) and four scratchpad probes
(`fs_b3_smoke` 12 Â· `fs_b3_ui` 26 Â· `fs_b3_goods` 15 Â· `fs_b3_idle` 23).
...
- world: medium 64 â†’ 90 and large 96 â†’ 136; the "road into water is rejected" probe LENDS
  ownership to a shore pair instead of hunting for water inside a radius-12 castle claim (the
  board is 30% water now, not 41%); the separation bar became "â‰¥18 steps on the worst seed" plus
  "never below the generator's own 22% floor" â€” the worst board is 20 steps where it used to be
  15, and the fraction and the walk stopped being the same statistic when the map doubled.
- transport: the busy-frame scan went 1200 â†’ 4000 ticks and reports which of its three conditions
  never arrived. The town is spread over twice the ground, so a fresh site's digger and builder
  have a correspondingly longer walk â€” `building` never reached 1 inside two game-minutes while
...
- economy: the stonecutter's near/far check now DRAINS the event stream by watermark instead of
  ...
  first â€” the full event list read [5650, 5651, 5651, 5287, 5287, 5288, 5288] â€” and the poll, out
...
- visuals: the sparkle layer left the scene-name list and became an absence assertion; the water
  ...
  to) and gained a paused-freeze and a 4Ã—-is-four-times case; the fish arc runs the sim too, and
...
- visual: the doorway fade is measured over the whole settlement rather than by latching onto the
  first serf caught mid-fade â€” he can finish and step back OUT inside the window (appear
  0.64 â†’ 0.45 â†’ 1, read as "never disappeared"), and a carrier walking into a WAREHOUSE is
  absorbed outright, so there is no end of fade to read on him; its warm-up went 2200 â†’ 7000
  ticks so something is GARRISONED. The audio one-for-one window went 1200 â†’ 5000 ticks after its
...
- ui: the connect-chip section became a removal assertion; the road-via-UI staging takes the
  ...
  vertex).
**Film strips regenerated** â€” 28 subjects, gait CV 0.0945-0.1066, jump 1.02Ã—, yaw 0, 0 page
errors â€” so they show the shipping default including the goods sprites and the new idles.
...
â€” those are boards whose near hills hold no minable rock at all, where the guarantee correctly
...
## ðŸ° FARMSTEAD â€” THE MEDIEVAL SKIN (2026-08-02, UI agent; render agent worked in parallel)
...
intact â€” cards grey IFF unaffordable, the fixed-height detail strip on selection, the
...
`fs-ui.js` was **not touched** â€” every class the re-skin needed was already in its markup.
...
- **OAK** = structure â€” frames, rails, panel bodies (build panel, dock, speed rail, minimap
  mount, dropdowns, the sheet's board).
- **PARCHMENT** = anything you READ â€” the stock tablets, the rate strip, build cards, the detail
  strip, the context ledger, sheet bodies, toasts, the steward's menu.
- **STONE** = anything you PRESS â€” every button face, chamfered light-above/dark-below, and it
  travels 2 px into its seat when held.
- **IRON STUDS + HEMP ROPE** = joinery â€” studs pin panels at the corners; a rope border-image
...
material â€” the knots in the oak are in the same place on every device. Seamless by construction:
grain lines are `sin()` over a whole number of periods across the tile, and anything that crosses
an edge is drawn nine times at Â±W/Â±H.
...
- **FAIL-SOFT, and PROVEN, not asserted in prose**: the suite loads the page with `fs-skin.js`
  ABORTED outright and plays on â€” panels fall back to solid oak/parchment/stone (all the bevels
...
Antiquaâ€¦`) in uppercase with real letterspacing carries the medieval note on headings, buttons
and tool names; **figures stay monospace** so a changing number can never shift its neighbour;
**dense body copy stays in the system sans** â€” a serif at 10.5 px in a 248 px phone panel costs
...
**SIX BUGS THE RE-SKIN FOUND â€” five of them older than the re-skin.** Each is now measured by
...
1. **Multi-layer `background-repeat` leaks.** The oak recipe declares FIVE background layers
   ...
   â†’ suite asserts every skinned surface TILES (the material is always the last layer).
2. **`repeat(3,1fr)` is `minmax(AUTO,1fr)`** â€” the auto floor is the longest unbreakable word, so
   ...
   â†’ suite measures the grid's own blowout.
3. **The detail strip is `overflow:hidden` in BOTH axes and only the vertical one was measured.**
   The head row (serif name beside the cost) overflowed on a phone and ate the price â€” "â€¦1 sto".
   The price is the reason the line exists. Head now wraps; strip 132â†’140 desktop / 112â†’126
   mobile, both re-measured by the suite walking every building. â†’ new horizontal-clip check.
4. **`.fs-ghost` / `.fs-primary` / `.fs-danger` are used STANDALONE** in the dialog markup (a
   modal button never carries `.fs-btn`), so they had no base geometry â€” a white OS button sat in
   the middle of the game-over screen. **PRE-EXISTING.** All three joined the stone recipe.
5. **`#title` was `justify-content:center` on an overflowing flex column** â€” which overflows at
...
6. **The rate strip's figures sat at 3.7â€“4.4:1** on a darkened parchment. They are NUMBERS. They
   keep the full parchment and earn their lower rank from being smaller, tighter and sunken.
   â†’ suite computes WCAG ratios in-page for seven text/surface pairs and gates at 4.5:1.

**THE TWO STATES THE RE-SKIN WAS NOT ALLOWED TO BLUR.**
- **Unaffordable = CHALKED PARCHMENT** â€” faded texture + grey ink + 0.72 opacity, done WITHOUT a
...
screen has none spare â€” six 40 px tools already span 240 px â€” so on mobile the rail is a 2 px lip
...
â€” two of the five icon-only tab chips were coming out 33 px wide on their emoji alone; five 40 px
...
co-op host+join/continue/hint/back) Â· top bar + ticker + rate strip + alert chip + bell + menu
button Â· speed rail Â· dock Â· build panel (tabs, linen well, cards, detail strip) Â· context ledger
Â· attack + demolish modals Â· game over Â· side menu Â· sheets (distribution steppers, priorities,
tools, knights, stats, save/load, settings, help) Â· toasts Â· minimap frame Â· touch long-press menu
Â· co-op chrome (chip, invite, ping, veil, host-left) Â· dev telemetry + legacy hint bar.

**SUITES** â€” `_verify-farmstead-ui.cjs` **149/149** (was 135; +14, all new: the six skin checks,
...
check; `#fsMinimapToggle` is excluded from the â‰¥39 px sweep (documented exception â€” it is the
...
`node tools/_fs_ui_shots.cjs [--before]` (20/20) â€” every shot ASSERTS the thing its filename
...
descriptions â€” that is the price of the anti-reflow contract and is deliberate; the rope frame is
...
## ðŸŒ¿ FARMSTEAD CHARM PASS â€” culling, fluffy trees, sprite grass (2026-08-02)
...
the flat surfaces with low cost grass and the trees should be fluffy and swaying in the windâ€¦
we just need the game to now have that medieval charm."* Render layer only â€” `fs-render.js`,
...
**1 Â· CHUNKED FRUSTUM CULLING â€” the thing everything else is paid for out of.**
...
`frustumCulled = false`. Instances are bucketed on a world grid (`CULL_CELL` 16u â†’ 304 buckets on
...
- **COMPACTION, NOT A MESH PER BUCKET, and that was a decision not a shortcut.** The obvious shape
  â€” an InstancedMesh per (kind, bucket) with three.js culling it â€” was costed and rejected: ~250
  buckets Ã— ~10 kinds in a developed patch would take a zoomed-out frame from 50 draw calls to
  ...
  Draw calls at play zoom: 51 â†’ 51. Zoomed out: 52 â†’ 49 (a pool with nothing visible stops
  drawing entirely).
- Authoritative per-slot matrices/colours live in CPU-side `src`/`srcCol`; `mesh.instanceMatrix`
  ...
  `cpSetMatrix`/`cpSetColor`/`cpAssign`/`cpRelease` â€” nothing else may touch it. Repack runs only
...
- Hooks: `FSRender.setCulling(on)` Â· `cullInfo()` Â· `cullVisibleAt(x,z)`. Decor rides the same
  buckets, so grass is culled by the same code.

**2 Â· FLUFFY TREES â€” the canopy is painted, the geometry is small.**
...
solid core, so the silhouette comes from paint â€” ragged, per-cell different, never twinned.
- ONE 4x2 atlas (`FSModels.foliageAtlas`, `LEAF_TEX_PX` 256) serves every tree because a tree is
  ...
  single constant uv** â€” zero uv derivative â†’ always mip 0 â†’ no amount of bleeding from the ragged
  ...
  Leaf masses are LOBED, not round â€” a circular card is the same stamp the icosahedra were.
- **THE CAMERA DECIDED THE ARRANGEMENT.** At 49-58Â° pitch a tree is read largely from above, so
...
- **CONIFERS KEEP CONES, and that is a finding.** A card-built pine fails on this camera both
  ...
  instead â€” a cone whose base ring is SCALLOPED (radius alternating full / ~0.7, jittered), so the
  plan view is a soft rosette of branch clusters instead of an octagon â€” for FEWER triangles than
...
- Triangles per tree: broadleaf mature **216 â†’ 92**; conifer 147 â†’ 146 (same count, much softer).
  All twelve stage silhouettes stay monotonically taller and wider â€” asserted, because a
...
**3 Â· SPRITE GRASS â€” the meadow is back, and the two things that made it "busy" are fixed.**
...
- **A clump takes its colour FROM THE GROUND IT STANDS ON** (`tuftTint` samples the terrain's own
  ...
  had the classic aliasing bug â€” the lerp TARGET was `tmpC`, which every caller passes as `out`,
  so it lerped a colour toward itself and silently did nothing.)
- **AUTHORED TALLER THAN WIDE** (`TUFT_H` 0.36 / `TUFT_W` 0.26). An upright quad at 52Â° loses
  ~62% of its HEIGHT to foreshortening and none of its width, so a clump wider than it is tall
  arrives on screen as a squat spiky rectangle â€” a fallen leaf, which is exactly what the first
...
- Placement: GRASS ONLY (swamp/desert dropped â€” the meadow's own cutout in a marsh reads as the
  meadow leaking into it), never on a claimed vertex, and **never on PAINTED ROAD** â€” the network
  ...
  population depend on how the world arrived (41 clumps of difference) â€” `syncClaims` does the
...
- **THE PER-FRAME CPU BREEZE IS GONE.** Grass sways in the same vertex shader as the trees
  (`swayMat(m, amp, mask)` gained a per-material `uSwayK` â€” x = share of the wind, y = the height
  at which it reaches full lean â€” so a 2.6u tree and a 0.36u tuft bend the same at the tip while
  ...
  `setTreeSway(false)` still stops the whole world dead, grass included â€” the film-strip rig is
  untouched and asserted.
- **ALPHA BLEED â€” the fix for why every cutout in the world went muddy at distance.** A canvas'
  ...
  transparent region with the sheet's own mean â†’ (103,134,62), a 20% lift, and the specks stop
...
- The blade-noise ground sheet the brief asked about ALREADY EXISTS (`FSModels.groundTex`,
  ...
  feathered edges â€” see `fs_charm_roadedge.png`, plus the existing decal/normals/emissive checks.

**PERF** â€” Large (W=136), seed 12345, 3 AIs, 3000 ticks, SwiftShader 1280x800, 60-frame medians,
...
**-71.9% triangles / -44% frame** at play zoom, **-64.4% / -40%** zoomed out â€” the case that was
...
21.7 ms of SwiftShader (29%) â€” over the 10% target *on a CPU rasteriser*, where alpha-tested fill
...
off in every number above. Frame times swing +-30% run to run under parallel-agent load â€” the
triangle and draw-call columns are the stable ones.

**TUNABLES** (all `FSC.VIS`): `CULL`/`CULL_CELL` 16/`CULL_PAD_XZ` 1.8/`CULL_PAD_UP` 4.0/
`CULL_PAD_DOWN` 0.8 Â· `LEAF_CARDS` [2,3,5,6]/`LEAF_CROWN_CARDS` [0,1,1,2]/`LEAF_CORE` 0.62/
`LEAF_CARD_TILT` 0.42/`LEAF_ALPHA_TEST` 0.42/`LEAF_TEX_PX` 256 Â· `TUFT_PER_VERTEX` 3/`TUFT_MAX`
30000/`TUFT_H` 0.36/`TUFT_W` 0.26/`TUFT_GROUND_MIX` 0.86/`TUFT_ROAD_ALPHA` 26/`TUFT_FADE_DIST`
66/`TUFT_SHADER_SWAY` 0.9/`TUFT_SWAY_MASK` 2.4 Â· `TREE_SWAY` 0.07 â†’ **0.09**/`TREE_SWAY_MASK` 0.55.

**SUITES**: `_verify-farmstead-visuals.cjs` **187/187** (was 155; +32) â€” new section 12 CULLING
...
snapshotting (it was comparing an un-synced live world against a rendered reloaded one â€” a test
artefact, and with both rendered the two agree clump for clump).
**SHOTS**: `node tools/_fs_charm_shots.cjs` **21/21** â†’ `shots/fs_charm_{forest_before,
...
reviewed film strips) â€” the script asserts it is on disk and not blank and regenerates the other
...
reports EVERY pixel as changed â€” stash the grab on `window` inside the browser instead.
`animDecor` re-asserts `mesh.visible` every frame, so hiding a decor pool from a test to see what
it contributes silently does nothing (set `TUFT_FADE_DIST` to 0 instead) â€” that cost a wrong
...
## ðŸŒ¾ FARMSTEAD BATCH #4 â€” seven items from one session at the wheel (2026-08-02)
...
made inert in co-op; and construction 1.5Ã— faster.

**0 Â· TWO BUGS THE BRIEF UNCOVERED, and both explain complaints nobody had connected.**
- **A DOUBLE-SIDED CARD WAS BEING LIT FROM BEHIND BY ITS OWN NEGATED NORMAL.** Every cutout in
  this world â€” grass clumps, leaf masses, needle fans â€” is authored with its normal pointing
  STRAIGHT UP so it takes the ground's light, and drawn `side: DoubleSide` because the camera
  orbits 360Â°. But r128's MeshLambert lights per VERTEX into `vLightFront` / `vLightBack` and
  ...
  â€” only the hemisphere's ground colour plus the emissive lift. Half of every clump and half
  ...
  share ONE compiled program (the cache key is EXTENDED â€” `fsTreeSway|bothLit` â€” not replaced,
...
- **`getImageData` ON A GPU-BACKED 2D CANVAS COSTS 1.4 SECONDS.** The new carpet normalises its
  own mean, which means reading itself back â€” and the world suite's terrain-build budget caught
  the result at **1,901 ms**. Measured: a 512Â² readback is 1,449 ms on a plain canvas against
  **19 ms** with `willReadFrequently`, and even a 32Ã—32 readback costs 439 ms, so the stall is
  the GPUâ†’CPU SYNC, not the pixels. `canvasTex` now creates every context with the hint. It was
  already being paid by the alpha bleed: `foliageAtlas` went 527 â†’ 26 ms and `groundTex`
  1,898 â†’ 68 ms, i.e. this batch made texture generation cheaper than it was before it started.

**1 Â· THE SEA OF GRASS.** The arithmetic is the whole design. At play zoom the camera puts ~24
screen px on a world unit laterally and ~19 along the ground, so one `GROUND_TEX_UV` tile
covered ~100Ã—80 px â€” and a 256-texel sheet stretched over it was being MINIFIED 2.5Ã—, which put
...
- **CLUMP scale** (0.2-0.6 world units = 5-14 screen px) â€” soft lumps with a shaded seat, one
  gradient each. This is what reads as a mass of grass; nothing at blade scale can.
- **BLADE scale** â€” 6,000 strokes per rank, dark stems then lit tips, for the close zoom where
  the sheet is magnified rather than minified.
- **SPECK scale** â€” 46,000 single pixels written straight into the buffer.
...
the ground, so its mean IS the world's brightness â€” contrast can go where the art wants it only
because the level is put back. `GROUND_TEX_PX` 256â†’**512**, `GROUND_TEX_UV` 4.2â†’**3.6**.
- **ANISOTROPY IS SET FROM THE RENDERER, NOT THE MODEL** (`applyGroundAniso`). The ground is the
  one surface seen at a 49-58Â° graze all the way to the fog and asks for 8; the maximum is a
...
- Tufts: `TUFT_PER_VERTEX` 3â†’**4**, `TUFT_H/W` 0.36/0.26 â†’ **0.45/0.325** (aspect kept â€” bigger
  clumps cover ~1.6Ã— the ground for the SAME triangle count, the cheapest coverage on the
  board). Grass-only, off roads, off claimed ground â€” all unchanged.

**2 Â· THE TREES ARE CLUMPS NOW** (reference: DOS-Settlers sprite trees). `leafMass` was a haze â€”
...
own heading. Rim blobs hang half outside the lobed radius â€” that is the silhouette, from the
same shapes as the interior. Value range 0.42â†’1.0; the reference's undersides are near-black.
Plus: `LEAF_CARDS` [2,3,5,6]â†’**[3,4,6,7]**, `LEAF_CROWN_CARDS` [0,1,1,2]â†’**[0,1,2,3]**, cards
squarer and their centres tighter (the old fan was authored 1.90Ã—1.55 of the crown radius and
read as a flat splat with a wide waist), `LEAF_A/LEAF_B` shifted olive, and `TREE_TRUNK`
darkened to 0x43331f/0x4e3b26 with the trunk thinner and shorter â€” in the reference the trunk
...
- **THE CONIFER'S CONES ARE PAINTED, and they carried NO uv at all before.** `firSkirt`
  ...
  flat-shaded lampshade. It emits uvs now (u round the skirt, v baseâ†’apex, per TRIANGLE so the
  wrap-around face cannot smear the whole cell across one lobe) and samples a NEW, FULLY OPAQUE
  atlas cell â€” the atlas went 4Ã—2 â†’ **4Ã—3** for it, because alpha on a cone is what laces a
  distant fir into nothing.
  **THE STRUCTURE RUNS ACROSS u, NOT DOWN v, and that is the finding.** Bands down v are
  compressed into a 0.4-unit tier â€” a couple of screen pixels, which averages straight back to
  ...
  its own light from above. Segments 8/10 â†’ **10/12** for two triangles a tier.
- The solid core is the DARKEST thing in the tree now (`deep Ã— 0.80`, `LEAF_CORE` 0.62â†’0.56,
  Ã—0.76 again on the young stages). At 36% of the way to the highlight it showed between the
  cards as a lit diamond â€” a wood full of bright green gems, caught on the review plate.

**3 Â· GOODS ARE CARRIED IN FRONT.** A new **carry** pose â€” the walk cycle with both arms out,
elbows apart, a straighter back and a constant arm pose across the cycle (a man carrying a crate
does not swing it) â€” baked for BOTH looks, plus a per-frame **`hands`** anchor resolved from the
...
- **THE MINIFIG GREW ARMS.** They were two more boxes inside the merged torso, so a minifig serf
  could not reach for anything. Parts flagged `arm: Â±1` build their own triplet and hang off a
  SHOULDER group; a part list with no arm flags (the knight) behaves exactly as before.
- **THE SKINNED DWARF'S SHOULDER AXIS IS MIRRORED â€” measured, not derived.** The carry pose
  ...
  has been BACKWARDS on that look since it was written â€” 17Â° on a 26 px sprite, which is why
...
- **A PIXEL CANNOT BE SPLIT BACK INTO HEIGHT AND DEPTH.** The `hands` anchor is a pixel inside
  the body cell, and a hand held FORWARD projects downward exactly like a hand held lower â€” so
  ...
  cell STANDS it on its anchor, so it is dropped `0.30 Ã—` its own quad or the plank floats at
  chest height.

**4 Â· WORK IS A TOOL SWING.** The work cycle was the torso alone â€” pitch forward by
`swing*0.42`, bob, brace â€” a man bowing at his work with his tool welded to his hip. The arms
raise the tool overhead (âˆ’0.30 â†’ âˆ’2.05 rad at the shoulder) and strike down through the bottom
of the arc; the torso lean halved, because a 24Â° bow ON TOP of a raised tool folds him in half.
**The arms SPLAY as they rise** (`armOut` 0.10 â†’ 0.55) and that is not decoration: the raise is a
rotation in the SAGITTAL plane, so from dead ahead or dead behind it is pure foreshortening and
the whole swing reads as a man standing still â€” which is exactly what the first bake's contact
sheet showed.
- **THE TOOL SWEEPS WITH THE HAND, on a baked-once overlay.** The manifest carries a per-cell
  **`toolAngle`**: the screen-space rotation the posed arm puts that cell's (arms-at-rest) tool
  image at. The renderer rotates the quad about its own pivot â€” the hand â€” through two new
...
- **THE ANGLE IS MEASURED AGAINST THE RIG'S OWN REST ARMS**, and the first cut got this wrong:
  referencing "the tool points up the body's +Y" is fine for the minifig and 155-180Â° out on the
  skinned dwarf, whose rest forearm points DOWN â€” every axe swung upside down. The bake poses
...
**5 Â· FOUR CONSTRUCTION STAGES, DERIVED.** A site used to raise ONE generic rising box whatever
it was going to become. By quartile: **0** cleared pad with the delivered planks and stones
heaped on it Â· **1** a TIMBER FRAME (corner posts, sill, top plate, studs, braces) Â· **2** WALLS
UP, NO ROOF Â· **3** â€¦plus the bare roof TRUSSES Â· then the finished model.
- **STAGES 2 AND 3 ARE THE FINAL MODEL WITH ITS ROOF CLASSIFIED OUT**, which is what makes 23
  building types tractable â€” nobody hand-models 92 meshes. `classifyBuilding` decides in order of
  confidence: the part's own `role` (which `shell()` now stamps â€” that is the roof of every
  ordinary building, exactly, by construction) â†’ its ATLAS CELL (thatch/shingle/slate/tile/straw
  are roofing and nothing else here is made of them) â†’ its COLOUR against the roof palette â†’
  ...
  from the part bounds â€” nothing is per-type.
- **CEILING JOISTS, because a wall is a solid box.** Taking the roof off leaves the box's TOP
  FACE looking at a camera 52Â° above it: a flat lid in the wall's own colour, which reads as
...
- The site mesh's ambient lift follows what it mostly IS (`COL.BLD_WALL` at stages 2-3, site
  timber below), or the walls changed colour on the frame the building finished â€” the same
  lesson the road decal learned about matching the ground it lies on.

**6 Â· THE SPEED RAIL IS TOP RIGHT, AND INERT IN CO-OP.** It sat centred under the top bar, which
...
- Multiplayer: the rail is greyed (`#fsSpeed.locked`, every button `disabled`, title "Co-op runs
  at 1Ã—") and the number keys and Space are inert with a blip toast. Speed multiplies every
  client's command LEAD (`CMD_DELAY_MP Ã— max(1, speed)`), so it is a property of the SESSION, not
...
- **THE GATE IS DELIBERATELY NOT INSIDE `setSpeed()`.** Putting it there broke lockstep outright,
  ...
  two counters differently â€” and `cmdSeq` is serialized state, so it is in the sync hash. The two
  ...
  the HOST's job alone, only for speed ABOVE 1 (pulling a PAUSED room to 1Ã— would fight every
  legitimate reason a connected sim sits at 0), latched on a tick boundary.

**7 Â· CONSTRUCTION IS 1.5Ã— FASTER.** `FSC.SWING_TICKS` [77,51,51,77] â†’ **[51,34,34,51]** (mean 64
â†’ 42.5 ticks = 1.506Ã—). Nothing else moved: the accumulator, `SWING_PER_MAT`, `LEVEL_T` and every
...
**PERF.** Large (W=136), seed 12345, 3 AIs, 3000 ticks, SwiftShader 1280Ã—800, 60-frame medians,
`readPixels` after every frame. Draw calls **51 â†’ 49** at play zoom and **49 â†’ 48** zoomed out;
triangles **134,616 â†’ 142,091** (+5.6%) and **164,214 â†’ 171,229** (+4.3%).
...
only honest number is one where both halves are measured in ONE page â€” the shipped art against
the same build with this batch's per-frame lever (the meadow) dialled back:
**play 88.2 â†’ 91.3 ms (+3.5%), zoom-out 98.6 â†’ 97.2 ms (âˆ’1.4%)**, tufts 19,759 â†’ 24,129. Both
inside the Â±15% gate. Texture generation is *faster* than before the batch (see Â§0).

**SUITES â€” all nine green, 1,044 checks, 0 page errors**: world **84** Â· transport **138** Â·
economy **107** Â· visuals **187** Â· visual **60** Â· military **124** Â· ui **149** Â· polish **68**
Â· mp **127** (+1: the co-op speed rule). Plus `node tools/_fs_b4_shots.cjs` **38/38**.
...
- visuals: the sway-hook check asks for the sway program key as a PREFIX (`litBothSides` extends
  ...
  carrier's PACK covers the small of his back â€” a serf caught at azimuth 7 moved 5 pixels where
  his own front view moves 486, and which serf the probe grabs moved when construction sped up;
  the culling pixel-parity tolerance went 1-in-20,000/worst 96 â†’ 1-in-10,000/worst 200, because
  both halves scale with the MEADOW and the meadow got denser and higher-contrast (measured
  54 / 1,024,000, worst 147).
- visual: `SIM_BASELINE` re-derived â€” construction speed is a rule change, so every seed's sim
...
- ui: the paused-queue toast check PAUSES FIRST, THEN CONNECTS â€” that state is still reachable
...
- mp: "a GUEST speed change moves BOTH screens to 2Ã—" became the new rule from the other side â€”
  the rail is visibly locked and inert on a connected client, a clock that arrives wrong is
  pinned back to 1Ã— on BOTH screens, and the live accumulator check runs at the speed the session
  actually has.
- harness: `browser.newPage` gets a 150 s default navigation timeout. Every page a suite opens is
  ...
  farmstead page never stops rendering â€” measured 0.9 s to boot a sibling against a fresh first
...
--out â€¦`; MINIFIG FIRST â€” the dwarf inherits its frustum, and it still fits, so px-per-unit,
footPx and the quad size are all unchanged): rows 23 â†’ **31**, cells 2,672 â†’ **2,928**, anchors
2,912 â†’ **3,920**. minifig 2.83 â†’ **3.39 MB**, dwarfknight 4.23 â†’ **5.00 MB**. sha256 prefixes:
...
/ `GROUND_CLUMPS` 620 / `GROUND_SPECKS` 46000 / `GROUND_TEX_MEAN` 0.955 / `GROUND_TEX_ANISO` 8 Â·
`TUFT_TINT_LIFT` 1.13 Â· `LEAF_CONIFER_MEAN` 0.94. Bake side: `B.ARM = {swing .34, carry 1.32,
carryOut .24, workLo .30, workHi 2.05}`, `DK.armSignX`.
**SHOTS**: `node tools/_fs_b4_shots.cjs` (38/38) â†’ `shots/fs_b4_{carpet,carpet_closeup,
trees_vs_ref,carry,builder_strike,woodcutter,stages_small,stages_large,speedbtns,speedbtns_mp,
zoomout}.png` â€” every plate asserts what its filename claims BEFORE it is written (the carpet
...
camera's 49-58Â° band he is partly behind its canopy from every yaw â€” the plate picks the clearest
