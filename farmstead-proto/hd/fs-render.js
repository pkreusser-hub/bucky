...
(function () {
  ...
  function terrainColor(v, out) {
    ...
    /* PROTO style hook — grade the palette BEFORE slope shading so the shading
       (a lightness modulation) survives the re-grade untouched. */
    if (window.FSGRADE) window.FSGRADE(out, t === FSC.TERR.WATER ? "waterbed" : "terrain");
  ...
  function rebuildRoads() {
    ...
    if (window.FSGRADE) { window.FSGRADE(base, "road"); window.FSGRADE(edge, "road"); }   /* PROTO style hook */
    let o = 0;
    const a = [0, 0], b = [0, 0];
    ...
    if (window.FSGRADE) window.FSGRADE(plank, "road");                  /* PROTO style hook */
    const seen = new Set();
