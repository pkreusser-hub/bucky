...
(function () {
  ...
  /**
   * The difficulty table sits OVER the personality (Farm Kart's DIFF_BEHAVIOR
   * pattern). Personality says who this opponent is; difficulty says how hard
   * they play. It is read from G — never from a client setting — so a lockstep
   * game cannot have the two machines running different opponents.
   */
  function diff(G) { return FSC.aiDiff(G && G.difficulty); }
  FSAI.diff = diff;
  ...
  function roomLeft(G, p) {
    ...
    for (let i = 0; i < land.length; i += stride) {
      const v = land[i];
      if (FSMap.bldBlocked(map, v) || map.flagAt[v] || map.terr[v] !== T.GRASS) continue;
      /* objRefuses, not objBlocks: this is a cheap PRE-FILTER for the real
       * canPlaceBuilding below, and since 2026-08-01 a tree is felled by the
       * construction rather than refusing it. A stricter pre-filter than the
       * predicate it guards would quietly under-count a forested kingdom's room
       * to grow — and the AI would feel hemmed in where the player no longer is. */
      if (FSMap.objRefuses(map.obj[v])) continue;
  ...
  function wishList(G, p, c, inv, st) {
    const per = persona(p), D = diff(G);
    const land = Math.round(has(c, "hut") + has(c, "tower") * 1.5 + has(c, "fortress") * 2);
    const want = (n) => Math.max(1, Math.round(n * per.expand * D.expand));
    const w = [];
  ...
  function pickSite(G, p, type, st) {
    ...
    for (let i = 0; i < land.length; i += stride) {
      ...
      if (FSMap.bldBlocked(map, v) || map.flagAt[v]) continue;
      if (def.mountain) {
      ...
      if (FSMap.objRefuses(map.obj[v])) continue;      // a tree is felled, not a refusal
  ...
  function prospect(G, p, st) {
    ...
    function rock(v) {
      ...
      FSMap.forRadius(map, v, FSC.GEO_RING[1], (u, d) => {
        ...
        if (map.sign[u] || map.flagAt[u] || FSMap.bldBlocked(map, u)) return;
        m++;
      });
      return m;
    }
  ...
  function warThink(G, p, st) {
    ...
    st.attackT = G.tick + Math.round(FSC.AI_ATTACK_T * D.attackT);
    const targets = FSMil.attackTargets(G, p);
    if (!targets.length) return false;
    const per = persona(p);
    // higher `need` = shyer: an easy opponent wants overwhelming odds first
    const need = FSC.AI_AGGRO * D.aggro / per.aggro;
    ...
    let n = Math.ceil(best.max * Math.min(1, FSC.AI_ATTACK_SHARE * D.share));
  ...
  function plan(G, p) {
    ...
    if (c._sites >= Math.max(1, Math.round(FSC.AI_MAX_SITES * D.build))) return "busy";
    ...
    const budget = Math.max(1, Math.round(FSC.AI_TRIES * D.tries));
    let tried = 0;
    for (let i = 0; i < wish.length && tried < budget; i++) {
      const type = wish[i];
  ...
  FSAI.tick = function (G) {
    ...
    // difficulty's coarsest lever: how OFTEN an opponent gets to think at all
    const period = Math.max(1, Math.round(FSC.AI_PERIOD * diff(G).period));
    for (let p = 0; p < G.players.length; p++) {
      const pl = G.players[p];
      if (!pl.isAI || pl.eliminated) continue;
      // staggered so two AIs never plan on the same tick
      if (((G.tick + p * 13) % period) !== 0) continue;
      plan(G, p);
    }
    return G;
  };
