...
(function () {
  ...
  function occLevels(G, b) {
    ...
    /* PLAYTEST 2026-08-01 — DIFFICULTY, defensive half. A computer opponent
     * whose border is under pressure mans its walls harder the harder it is
     * playing: `answer` adds occupancy LEVELS on threatened buildings only, so
     * an easy AI garrisons its frontier exactly as it always did while a hard
     * one answers a raid instead of watching it. Humans are never touched. */
    if (pl && pl.isAI && t > 0) {
      const bump = FSC.aiDiff(G.difficulty).answer * (t >= 3 ? 2 : 1);
      lo += bump; hi += bump;
    }
    return [Math.max(0, Math.min(FSC.OCC_LEVEL_MAX, lo)), Math.max(0, Math.min(FSC.OCC_LEVEL_MAX, hi))];
  }
  ...
  function slotVertex(G, flagV, i) {
    ...
    for (let d = 0; d < 6; d++) {
      ...
      if (u < 0 || !FSMap.walkable(map.terr[u]) || FSMap.bldBlocked(map, u)) continue;
      ring.push(u);
    }
