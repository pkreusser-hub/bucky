 ...
 *     shipped MIDI theme (assets/farmstead/music/02-settlers.mid) played via
 *     FSMidi on a softsynth loop. Users may still override with their OWN
 *     audio file via the file picker (never uploaded — see MUSIC SOURCE).
 ...
 *   clearCustomMusic() -> Promise<true>                 — back to the built-in MIDI
...
(function () {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // ===== MUSIC — built-in MIDI theme (+ optional custom file / synth) ======
  // ═══════════════════════════════════════════════════════════════════════
  // MUSIC SOURCE ABSTRACTION: 'midi' (shipped Settlers theme, default) |
  // 'file' (the user's own decoded track) | 'synth' (procedural fallback if
  // the MIDI failed to load). Same onGameStart/onGameEnd/musicOff plumbing
  // for all three — swapping the source is just which node graph feeds
  // musicGain.
  const BUILTIN_MIDI_URL = "assets/farmstead/music/02-settlers.mid";
  const BUILTIN_MIDI_NAME = "Settlers theme";
  let musicSource = "midi";
  ...
  let midiSong = null;            // parsed FSMidi song (null until fetch+parse)
  let midiPlayer = null;          // active FSMidi.play controller
  let midiLoadState = "idle";     // idle | loading | ready | error

  function ensureBuiltinMidi() {
    if (midiSong || midiLoadState === "loading" || midiLoadState === "ready") return;
    if (!window.FSMidi) { midiLoadState = "error"; musicSource = musicSource === "midi" ? "synth" : musicSource; return; }
    midiLoadState = "loading";
    fetch(BUILTIN_MIDI_URL).then((r) => {
      if (!r.ok) throw new Error("midi fetch " + r.status);
      return r.arrayBuffer();
    }).then((buf) => {
      midiSong = window.FSMidi.parse(buf);
      midiLoadState = "ready";
      if (musicSource === "midi" && musicArmed && ctx) restartMusicIfArmed();
    }).catch(() => {
      midiLoadState = "error";
      if (musicSource === "midi") {
        musicSource = "synth";
  ...
  function stopMusicNodes() {
    ...
    if (midiPlayer) { try { midiPlayer.stop(); } catch (e) { /* noop */ } midiPlayer = null; }
    synthMusicOn = false;
  ...
  function startMusicNodes() {
    ...
    } else if (musicSource === "midi" && midiSong && window.FSMidi) {
      midiPlayer = window.FSMidi.play(ctx, musicGain, midiSong, { loop: true });
    } else if (musicSource === "midi") {
      ensureBuiltinMidi(); // still loading — synth keeps the world from going silent
      synthMusicOn = true;
    } else {
      synthMusicOn = true;
    }
  ...
  FSAudio.musicInfo = function () {
    const ready = musicSource === "synth"
      ...
      || (musicSource === "midi" && !!midiSong);
    const name = musicSource === "file" ? customName
      : (musicSource === "midi" ? BUILTIN_MIDI_NAME : null);
    return { source: musicSource, ready: ready, name: name, midiLoad: midiLoadState };
  };
  ...
  FSAudio.clearCustomMusic = function () {
    return idbClearMusic().catch(() => {}).then(() => {
      ...
      musicSource = midiSong || midiLoadState !== "error" ? "midi" : "synth";
      ensureBuiltinMidi();
      restartMusicIfArmed();
      return true;
    });
  ...
  FSAudio.frame = function (dt, G) {
    ...
    if (ctx) {
      if (midiPlayer) {
        try { midiPlayer.tick(ctx.currentTime); } catch (e) { /* noop */ }
        // once the real MIDI is driving, drop the temporary synth filler
        if (synthMusicOn && musicSource === "midi") {
          synthMusicOn = false;
          if (padVoices) { stopPad(padVoices); padVoices = null; }
        }
  ...
  FSAudio.init = function (FSref) {
    FS = FSref;
    FSMap = FS && FS.FSMap;
    ensureBuiltinMidi(); // fetch+parse early (no AudioContext needed)
    return FSAudio;
  };
  FSAudio.debug = function () {
    return {
      ...
      midiLoad: midiLoadState,
      midiReady: !!midiSong,
      midiPlaying: !!midiPlayer,
      midi: midiPlayer ? midiPlayer.debug() : null,
