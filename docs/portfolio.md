...
- **Cash is generated by the real estate and reinvested into Schwab — the engine models
  it.** Growth (appreciation of the marks) and yield (distributions paid into cash) are
  separate per-class assumptions, and a sweep reinvests cash above a buffer (default $250k)
  into Public markets each quarter. Quarter order: grow → income to cash → contributions →
  withdrawals (cash first) → sweep → fee on the end balance. Evidence for the 5% Base yield
  on MIMG: cash + Schwab grew $917k from 4/13/25 to 7/10/26 (~$740k/yr combined), far more
  than Schwab's own market gains explain — ≈$600k/yr of distributions ≈ 5% of the MIMG
  base, while MIMG's marks appreciated ~4%/yr on top. This is also why Cash and Schwab get
  no implied-growth rate in the asset table: their moves are internal transfers.
