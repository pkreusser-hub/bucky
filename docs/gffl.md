...
## GFFL — weekly recap newsletter (2026-09-15)

Perry: recap the week's fantasy results, send a
polished email to the league, recurring, witty
sports-desk voice, no typical AI column.

`netlify/functions/gfflnewsletter.mjs` is a scheduled
function in the leaguecron shape. After
`LG.finalizeWeek` writes a real (non-void) weekly
doc, the next Tue/Wed/Thu 8:00 AM Central slot
builds a column and emails it through the existing
EmailJS service/template. Oldest unsent finalized
week first; create-only `gffl_nl_<fam>/<season>_w<N>`
so a week mails once. A Tuesday before the week is
official is a no-op (`no-finalized-week`); Wednesday
and Thursday are the MNF / nobody-opened-the-app
backups.

Who gets it is not a guessed list: family profile
rows with an address whose name matches a team's
`owner` / `claimedBy`, or the documented
choreUser → team map (dad→1, isaac→12, grandpa→3,
mom→5). Extra addresses only via
`GFFL_NEWSLETTER_TO`. No address, no send
(`no-recipients`).

Voice: a deterministic columnist is the default
(scheduled functions cap at 30s; Grok's measured
league TTFB has already kissed that). Closest finish
leads when it is inside 3 points; every pairing and
every score must appear; `slopLint` rejects the
usual AI-column tells and emoji. An optional
Grok/Sonnet hop (same keys as farmgpt) may replace
the fallback only when it returns in 10s and passes
both `slopLint` and `columnUsesFacts`.

EmailJS Account → Security must allow API calls
from non-browser apps (their default is off). Set
`EMAILJS_PRIVATE_KEY` if that account requires the
private key. Sends are serialized at 1/s to match
their rate limit.

Files: `netlify/functions/gfflnewsletter.mjs`,
`netlify.toml`, `tools/_verify-gfflnewsletter.mjs`,
this file.

**VERIFY:** `node tools/_verify-gfflnewsletter.mjs`
**84/84**. `node tools/_verify-leaguecron.mjs` still
**45/45** (the new toml block does not disturb it).
Bite (suite kept, `gfflnewsletter.mjs` + the
newsletter toml block at `origin/main`): **0/5** —
HEAD has no function, no schedule, and the restaged
"declared exactly once" count is zero (the old
negative lookahead passed vacuously on a missing
block; reason is at the check). App files restored,
hashes identical.
---
